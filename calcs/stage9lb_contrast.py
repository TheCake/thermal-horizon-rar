"""STAGE 9L-b -- THE CALIBRATED CONTRAST STATISTIC (correction arc leg 1;
2026-09-03; successor to the vetoed 9L E[fpm] meter -- see
data/stage9l_nullinj.txt: INSTRUMENT-NULL, trap #27).

THE STATISTIC: S = ln P(fpm >= 2.1) - ln P(fpm <= 1.8), computed from the
ARCHIVED marginalization (host prior LNPI over fcomp; sum over ffly/kw/ws2;
joint (fpm, sq) -> fpm marginal), per RUWE quartile. PRIMARY = Q1 (the
cleanest quartile, the load-bearing claim site); co-reads Q2-Q4 + pooled.
A FLAT posterior gives S = 0 EXACTLY and cannot pass the power gate below
(trap-#27 compliance: the bar is a value an uninformative posterior cannot
reach).

CALIBRATION (trap #22: between-draw scatter MEASURED): truth ladder
{1.0, 1.2, 1.5, 1.8, 2.1, 2.4} x R = 64 multinomial replications x 2
population seeds (31/101), injected at the archived G9L-3 cell
(fcomp = 0.20, ffly = 0.05, kw = 1.0, sq = 0.0), drawn at the Q1 narrow
count. Draw rngs: default_rng([9, seed, truth_index]), sequential draws.
CO-READS (report-only, risk-axis rule): injection at sq = 0.2 (truths
1.0/2.4); model eta = 1.3 (sky S + truths 1.0/2.4). The sky read comes
from the SAVED 9L tables (data/stage9l_tables_{seed}.npz) -- no sky
recompute. The 9P mass-model envelope re-statement = a successor leg
(9P-b), conditional on B-NOISE-BOUNDED; this stage reads the fiducial
mass table only.

GATES (each names the clause its failure vetoes -- the R35 standing rule):
  GB9LB-0 cache identity: the log-pp cache reproduces direct eval_block_nb
    on one synthetic dataset with max|dT| <= 1e-6 (lnL grade). FAIL ->
    wiring error; STOP, fix, rerun (vetoes everything).
  GB9LB-1 reader identity: the saved sky tables + stored LNPI reproduce
    the archived E[fpm] (seed 31: 2.21/2.17/2.26/2.12; seed 101:
    2.13/2.27/2.25/2.30) to +-0.01, and stored LNPI matches the rebuilt
    LNPI to 1e-9. FAIL -> sky read invalid; STOP.
  GB9LB-2 monotonicity: mean S(truth) increases along the ladder in both
    seeds, tolerating adjacent-rung inversions smaller than 1 pooled SE
    (= sd/sqrt(R)); any larger inversion FAILS -> vetoes the exclusion
    clause; letter forced B-POWER-DEAD.
  GB9LB-3 power: D = mean S(2.4) - mean S(1.0) >= 2 sigma_pool (rms of
    the six per-truth SDs) in both seeds. FAIL -> vetoes any bound;
    letter forced B-POWER-DEAD. (A flat posterior gives D = 0.)

LETTER (pre-signed, ordered, read on PRIMARY Q1, BOTH seeds;
z(t) = (S_sky - mean S(t)) / SD(t)):
  B-POWER-DEAD: GB9LB-2 or GB9LB-3 failed -> the narrow bin cannot
    measure fpm at catalog grade; the error claim moves ENTIRELY to the
    joint fits; the note is withdrawn or reframed onto them.
  B-NOISE-BOUNDED: z(1.0) >= 2 in both seeds -> the boost-free bin
    demands inflation; the quotable bound = the LARGEST ladder truth t
    with z(t) >= 2 in both seeds, quoted as "effective inflation > t at
    2-sigma calibration grade (per population realization; injection-cell
    conditional, co-reads shipped)". NEVER quote an E[fpm] point value.
  B-NULL-CONSISTENT: z(1.0) < 1 in either seed -> the narrow bin is
    consistent with formal errors; the note's claim is WITHDRAWN
    (joint-fit language only) and the 9L fork reads toward BOOST-EATEN
    pending the review round.
  B-GRAY: otherwise (min z(1.0) in [1, 2)) -> lean-grade language only;
    no bound quoted; note withdrawn or reframed onto the joint fits.

NO CREDENCE MOVEMENT from this stage under ANY letter: the 9L
credence-map question (56 -> 53) is PARKED for the author + the review
round per the 2026-09-03 finding. Output: data/stage9lb_contrast.txt +
data/stage9lb_calib.npz (calibration distributions for the review round).
"""

import math
import numpy as np, time
from astropy.io import fits
from scipy.special import logsumexp

L_ = []
def P(s):
    print(s, flush=True)
    L_.append(s)

src = open('calcs/stage2b_population.py').read()
ns = {}
exec(src.split('# ---------- validation')[0], ns)
run = ns['run']
GM = 4*np.pi**2; A0_CAN = 7.99e-7

def load_tab(path):
    t = np.load(path); y, b = t[0][::-1], t[1][::-1]
    lny = np.log(y); lny_u = np.linspace(lny[0], lny[-1], 512)
    return lny_u, np.interp(lny_u, lny, b)
LNY_U, TAB_S = load_tab('data/efe_boost_simple_g1p2.npy')
_,     TAB_B = load_tab('data/efe_boost_be_g1p2.npy')
LNY0, DLNY = LNY_U[0], LNY_U[1]-LNY_U[0]

d = fits.open('data/edr3_binaries.fits.gz', memmap=False)[1].data
plx1, plx2 = d['parallax1'], d['parallax2']
eplx1, eplx2 = d['parallax_error1'], d['parallax_error2']
sep, Rch = d['sep_AU'], d['R_chance_align']
G1m, G2m = d['phot_g_mean_mag1'], d['phot_g_mean_mag2']
plx = 0.5*(plx1+plx2)
MG1 = G1m+5*np.log10(np.maximum(plx1, 1e-6))-10
MG2 = G2m+5*np.log10(np.maximum(plx2, 1e-6))-10
sigv = 4.74047/plx*np.sqrt(d['pmra_error1']**2+d['pmdec_error1']**2
                           + d['pmra_error2']**2+d['pmdec_error2']**2)
ok = (Rch < 0.01) & (plx1 > 5) & (plx2 > 5) \
   & (plx1/np.maximum(eplx1, 1e-6) > 20) & (plx2/np.maximum(eplx2, 1e-6) > 20) \
   & (np.abs(plx1-plx2) < 3*np.hypot(eplx1, eplx2)) \
   & (sep > 200) & (sep < 50000) \
   & (MG1 > 2.6) & (MG1 < 14.2) & (MG2 > 2.6) & (MG2 < 14.2) & (sigv < 0.03)
MG_T = np.array([2.6,3.4,4.2,4.8,5.4,6.0,6.83,7.57,8.16,8.82,9.29,10.05,
                 11.21,12.45,14.26])
MS_T = np.array([1.60,1.33,1.12,1.00,0.90,0.82,0.70,0.64,0.57,0.50,0.44,
                 0.37,0.23,0.162,0.102])
Mtot_d = np.interp(MG1[ok],MG_T,MS_T)+np.interp(MG2[ok],MG_T,MS_T)
s_d = sep[ok]/1e3
dec_m = np.deg2rad(0.5*(d['dec1']+d['dec2']))
dra = (d['ra2']-d['ra1']+180.0) % 360.0 - 180.0
sx_ = dra*np.cos(dec_m); sy_ = d['dec2']-d['dec1']
sn_ = np.maximum(np.hypot(sx_, sy_), 1e-12)
sux_, suy_ = sx_/sn_, sy_/sn_
_cn = list(d.columns.names)
def _pick(*names):
    for n in names:
        if n in _cn: return np.asarray(d[n], dtype=np.float64)
    raise KeyError(names)
r1_ = _pick('radial_velocity1', 'dr2_radial_velocity1', 'rv1')
r2_ = _pick('radial_velocity2', 'dr2_radial_velocity2', 'rv2')
try:
    er1_ = _pick('radial_velocity_error1', 'dr2_radial_velocity_error1')
    er2_ = _pick('radial_velocity_error2', 'dr2_radial_velocity_error2')
except KeyError:
    er1_ = np.full(len(r1_), 2.0); er2_ = np.full(len(r2_), 2.0)
h1_, h2_ = np.isfinite(r1_), np.isfinite(r2_)
w1_ = np.where(h1_, 1.0/np.maximum(er1_, 0.5)**2, 0.0)
w2_ = np.where(h2_, 1.0/np.maximum(er2_, 0.5)**2, 0.0)
rvs_ = (np.where(h1_, r1_, 0.0)*w1_ + np.where(h2_, r2_, 0.0)*w2_) \
       / np.maximum(w1_+w2_, 1e-12)
th_ = sep/(2.06265e8/plx)
pmcor_ = rvs_*th_*plx/4.74047
vx_ = d['pmra2']-d['pmra1'] + pmcor_*sux_
vy_ = d['pmdec2']-d['pmdec1'] + pmcor_*suy_
vt_d = (4.74047/plx[ok]*np.hypot(vx_[ok], vy_[ok]))/(0.9417*np.sqrt(Mtot_d/s_d))
e_vx = np.sqrt(d['pmra_error1']**2+d['pmra_error2']**2)
e_vy = np.sqrt(d['pmdec_error1']**2+d['pmdec_error2']**2)
cosg = np.abs(sx_*vx_+sy_*vy_)/np.maximum(np.hypot(sx_,sy_)*np.hypot(vx_,vy_),
                                          1e-12)
gam_d = np.degrees(np.arccos(np.clip(cosg, 0, 1)))[ok]
sig_c = ((4.74047/plx)*np.sqrt(0.5*(e_vx**2+e_vy**2)))
SBINS = [(0.2,2),(2,6),(6,20),(20,50)]
# 9L amendment-1 standard: the meter reads the 0.2-2 kAU bin ALONE
NB_BINS = 1
SC2 = (np.array([0.63, 3.46, 11.0, 31.6])/31.6)**2
VE = np.logspace(np.log10(0.02), np.log10(6.0), 21)
GE = np.linspace(0, 90, 7)
NV, NG = 20, 6
vcen = np.sqrt(VE[:-1]*VE[1:]); gcen = 0.5*(GE[:-1]+GE[1:])
FLY = np.zeros((NV, NG))
for i in range(NV):
    for j in range(NG):
        FLY[i,j] = (1.0 if (0.5 <= vcen[i] <= 3.0) else 0.0) \
                   * max(30.0-gcen[j], 0.0)
FLY /= max(FLY.sum(), 1e-12)
UNI = np.ones((NV, NG))/(NV*NG)
sig_ok = sig_c[ok]
vc_ok = 0.9417*np.sqrt(Mtot_d/s_d)

def build_stratum(pm):
    D2, PLs, UB, FB = [], [], [], []
    for b in SBINS:
        m = (s_d>=b[0])&(s_d<b[1])&pm
        h,_,_ = np.histogram2d(np.clip(vt_d[m],0.021,5.9), gam_d[m],
                               bins=[VE, GE])
        D2.append(h.astype(float))
        PLs.append(sig_ok[m])
        cutp = 2.978/np.sqrt(s_d[m]) + 2.8284*sig_ok[m]
        acc = np.array([(vcen[i]*vc_ok[m] <= cutp).mean()
                        if m.sum() else 0.0 for i in range(NV)])
        for tpl, store in ((UNI, UB), (FLY, FB)):
            t = tpl*acc[:,None]
            store.append(t/max(t.sum(), 1e-12))
    ND = [int(h.sum()) for h in D2]
    return D2, PLs, UB, FB, ND

N = 500_000
FCOMP_GRID = np.array([0.0, 0.10, 0.20, 0.35, 0.50, 0.70])
FC0 = 0.10
FFLY_GRID = np.array([0.05, 0.10])
FPM_GRID = np.array([1.2, 1.5, 1.8, 2.1, 2.4, 3.0])
KW_GRID = np.array([0.7, 1.0, 1.4])
SQ_GRID = np.array([0.0, 0.1, 0.2, 0.3])
WS2_GRID = np.array([0.0, 0.045])
SEEDS = (31, 101)

pz = np.load('data/stage7jz_prior.npz', allow_pickle=True)
fg, lp = pz['fh_grid'], pz['lnpi_host']
sup = lp > -1e8
gband = pz['conv_band']/0.30
GS = np.linspace(float(gband[0]), float(gband[1]), 25)
fhmin, fhmax = fg[sup].min(), fg[sup].max()
LNPI = np.full(len(FCOMP_GRID), -1e9)
for gi in GS:
    fh_eq = FCOMP_GRID/gi
    m = (fh_eq >= fhmin) & (fh_eq <= fhmax)
    cand = np.full(len(FCOMP_GRID), -1e9)
    cand[m] = np.interp(fh_eq[m], fg, lp)
    LNPI = np.maximum(LNPI, cand)

def build_pop(seed):
    rng = np.random.default_rng(seed)
    p = {}
    u = rng.random(N); lo, hi, g = 0.15, 60.0, -0.6
    p['a_s']  = ((lo**g+u*(hi**g-lo**g))**(1/g))*1e3
    p['u_e']  = rng.random(N)
    p['psi0'] = rng.random(N)*2*np.pi
    nrm = rng.normal(size=(N,3)); nrm /= np.linalg.norm(nrm,axis=1,keepdims=True)
    p['f_ip'] = np.sqrt(np.clip(1-nrm[:,0]**2,0,1))
    p['M_s']  = 0.6+1.8*rng.random(N)
    p['uph']  = rng.random(N)
    xhat = np.zeros((N,3)); xhat[:,0]=1
    ef = xhat-nrm*nrm[:,[0]]
    ef /= np.maximum(np.linalg.norm(ef,axis=1,keepdims=True),1e-12)
    p['ef'] = ef; p['e2'] = np.cross(nrm,ef)
    los = rng.normal(size=(N,3)); los /= np.linalg.norm(los,axis=1,keepdims=True)
    p['los'] = los
    p['u_mix'] = rng.random(N)
    p['pick'] = [rng.integers(0, max(len(noise_pool[bi]),1), N)
                 for bi in range(len(SBINS))]
    p['gn1'], p['gn2'] = rng.normal(size=N), rng.normal(size=N)
    M_h = 0.5*p['M_s']
    p['comp'] = {}
    for k in (1, 2):
        u_q = rng.random(N)
        q = 0.1+0.9*u_q
        logP = rng.normal(5.03, 2.28, N)
        P_yr = 10**logP/365.25
        a_in = (M_h*(1+q)*P_yr**2)**(1/3)
        valid = (a_in < 130.0) & (a_in < p['a_s']/5.0)
        v_orb = 29.78*np.sqrt(M_h*(1+q)/np.maximum(a_in,1e-3))
        u_ = np.pi*2.83/np.maximum(P_yr, 1e-9)
        S = np.where(u_ < 1e-2, 1.0 - u_*u_/10.0,
                     3.0*(np.sin(u_) - u_*np.cos(u_))
                     / np.maximum(u_, 1e-300)**3)
        MGp = np.interp(-np.clip(M_h, MS_T[-1], MS_T[0]), -MS_T, MG_T)
        MGs = np.interp(-np.clip(q*M_h, MS_T[-1], MS_T[0]), -MS_T, MG_T)
        l_ = 10**(-0.4*(MGs-MGp))
        wfac = np.abs(q/(1+q) - l_/(1+l_))
        w = wfac*v_orb*S/4.74047*valid
        wd = rng.normal(size=(N,3)); wd /= np.linalg.norm(wd,axis=1,keepdims=True)
        p['comp'][k] = dict(w=w, wd=wd, uc=rng.random(N),
                            mh=q*M_h*valid, P=P_yr)
    p['gs'] = rng.normal(size=N)
    return p

def e_of_x(p, eta, wr):
    al = np.interp(np.log10(p['a_s']), np.log10([100,500,1000,50000]),
                   [0.6, 1.0, eta, eta])
    e_pow = 0.95*p['u_e']**(1/(1+al))
    # amendment-1 standard: BIT-VERBATIM marginal expression
    erf = 0.95
    e_rad = erf+(0.995-erf)*p['u_e']
    return np.where(p['u_mix'] < wr, e_rad, e_pow)

def vp_c(p, e_s, tab_a):
    a_s, M_s = p['a_s'], p['M_s']
    rp, ra = a_s*(1-e_s), a_s*(1+e_s)
    xg, wg = np.polynomial.legendre.leggauss(32)
    lo_, hi_ = np.log(rp), np.log(ra)
    lr = 0.5*(hi_-lo_)[:,None]*xg[None,:] + 0.5*(hi_+lo_)[:,None]
    r = np.exp(lr)
    gN = GM*M_s[:,None]/r**2
    bst = np.interp(np.log(gN/A0_CAN), LNY_U, tab_a, right=1.0)
    dPhi = np.sum(wg[None,:]*bst*gN*r, axis=1)*0.5*(hi_-lo_)
    return np.sqrt(np.maximum(2*dPhi/(1-(rp/ra)**2), 0))

def project(p, o):
    ef, e2, los = p['ef'], p['e2'], p['los']
    s3 = o[:,0,None]*ef+o[:,1,None]*e2
    v3 = o[:,2,None]*ef+o[:,3,None]*e2
    ssky = s3-los*np.sum(s3*los,axis=1,keepdims=True)
    vsky = v3-los*np.sum(v3*los,axis=1,keepdims=True)
    smag = np.linalg.norm(ssky,axis=1)
    b1 = ssky/np.maximum(smag[:,None],1e-12)
    b2 = np.cross(los, b1)
    b2 /= np.maximum(np.linalg.norm(b2,axis=1,keepdims=True),1e-12)
    vpar = np.sum(vsky*b1,axis=1)
    vper = np.sum(vsky*b2,axis=1)
    return smag, vpar, vper

def eval_block_nb(p, prj, D2, PLs, UB, FB):
    """VERBATIM eval_block_g restricted to the NARROW bins 0-1."""
    smag, vpar, vper = prj
    s_kau = smag/1e3
    out = np.zeros((len(FCOMP_GRID), len(FFLY_GRID), len(FPM_GRID),
                    len(KW_GRID), len(SQ_GRID), len(WS2_GRID)))
    for bi in range(NB_BINS):
        b = SBINS[bi]
        idx = np.where((s_kau>=b[0])&(s_kau<b[1]))[0]
        if len(idx) < 500 or len(PLs[bi]) == 0: continue
        vc = 2*np.pi*np.sqrt(p['M_s'][idx]/smag[idx])
        sg0 = PLs[bi][p['pick'][bi][idx] % len(PLs[bi])]/4.74047
        g1_i, g2_i = p['gn1'][idx], p['gn2'][idx]
        sk_i = s_kau[idx]
        gk_full = p['gs'][idx]
        for fi, fcm in enumerate(FCOMP_GRID):
            cvp = np.zeros(len(idx)); cvq = np.zeros(len(idx))
            mh_tot = np.zeros(len(idx))
            for k in (1, 2):
                c = p['comp'][k]
                act = c['uc'][idx] < fcm
                mh_tot += act*c['mh'][idx]
                cvp += act*c['w'][idx]*c['wd'][idx,0]
                cvq += act*c['w'][idx]*c['wd'][idx,1]
            boost = np.sqrt(1+mh_tot/p['M_s'][idx])
            for ki, kwv in enumerate(KW_GRID):
                vp_a = vpar[idx] + kwv*cvp
                vq_a = vper[idx] + kwv*cvq
                for pi, fpm in enumerate(FPM_GRID):
                    for wi, ws in enumerate(WS2_GRID):
                        if ws == 0.0:
                            vp_n = vp_a*boost + g1_i*sg0*fpm
                            vq_n = vq_a*boost + g2_i*sg0*fpm
                        else:
                            sig_eff = np.sqrt((sg0*fpm)**2
                                              + (ws/4.74047)**2)
                            vp_n = vp_a*boost + g1_i*sig_eff
                            vq_n = vq_a*boost + g2_i*sig_eff
                        vmag = np.hypot(vp_n, vq_n)
                        keep = vmag*4.74047 <= (2.978/np.sqrt(sk_i)
                                                + 2.8284*sg0*4.74047)
                        vtn = (vmag/vc)[keep]
                        gmn = np.degrees(np.arccos(np.clip(
                            np.abs(vp_n[keep])
                            / np.maximum(vmag[keep], 1e-12), 0, 1)))
                        gk = gk_full[keep]
                        for si, sqv in enumerate(SQ_GRID):
                            vts = vtn*np.exp(sqv*gk)
                            h,_,_ = np.histogram2d(
                                np.clip(vts,0.021,5.9), gmn,
                                bins=[VE, GE])
                            p0 = np.maximum(h/max(h.sum(),1), 1e-5)
                            p0 /= p0.sum()
                            for yi, ff in enumerate(FFLY_GRID):
                                wch = min(FC0*SC2[bi], 0.5)
                                wfl = min(ff*SC2[bi], 0.5)
                                wtot = min(wch+wfl, 0.6)
                                mixc = (wch*UB[bi]
                                        + wfl*FB[bi])/(wch+wfl)
                                pp = (1-wtot)*p0 + wtot*mixc
                                out[fi, yi, pi, ki, si, wi] += \
                                    np.sum(D2[bi]*np.log(pp))
    return out

def build_logpp_cache(p, prj, PLs, UB, FB):
    """Clone of eval_block_nb (bin 0 only) that STORES log(pp) per cell
    instead of contracting with D2 -- so each calibration replication is
    one tensordot. Gated against direct eval_block_nb by GB9LB-0."""
    smag, vpar, vper = prj
    s_kau = smag/1e3
    C = np.zeros((len(FCOMP_GRID), len(FFLY_GRID), len(FPM_GRID),
                  len(KW_GRID), len(SQ_GRID), len(WS2_GRID), NV, NG))
    bi = 0
    b = SBINS[bi]
    idx = np.where((s_kau>=b[0])&(s_kau<b[1]))[0]
    assert len(idx) >= 500 and len(PLs[bi]) > 0
    vc = 2*np.pi*np.sqrt(p['M_s'][idx]/smag[idx])
    sg0 = PLs[bi][p['pick'][bi][idx] % len(PLs[bi])]/4.74047
    g1_i, g2_i = p['gn1'][idx], p['gn2'][idx]
    sk_i = s_kau[idx]
    gk_full = p['gs'][idx]
    for fi, fcm in enumerate(FCOMP_GRID):
        cvp = np.zeros(len(idx)); cvq = np.zeros(len(idx))
        mh_tot = np.zeros(len(idx))
        for k in (1, 2):
            c = p['comp'][k]
            act = c['uc'][idx] < fcm
            mh_tot += act*c['mh'][idx]
            cvp += act*c['w'][idx]*c['wd'][idx,0]
            cvq += act*c['w'][idx]*c['wd'][idx,1]
        boost = np.sqrt(1+mh_tot/p['M_s'][idx])
        for ki, kwv in enumerate(KW_GRID):
            vp_a = vpar[idx] + kwv*cvp
            vq_a = vper[idx] + kwv*cvq
            for pi, fpm in enumerate(FPM_GRID):
                for wi, ws in enumerate(WS2_GRID):
                    if ws == 0.0:
                        vp_n = vp_a*boost + g1_i*sg0*fpm
                        vq_n = vq_a*boost + g2_i*sg0*fpm
                    else:
                        sig_eff = np.sqrt((sg0*fpm)**2
                                          + (ws/4.74047)**2)
                        vp_n = vp_a*boost + g1_i*sig_eff
                        vq_n = vq_a*boost + g2_i*sig_eff
                    vmag = np.hypot(vp_n, vq_n)
                    keep = vmag*4.74047 <= (2.978/np.sqrt(sk_i)
                                            + 2.8284*sg0*4.74047)
                    vtn = (vmag/vc)[keep]
                    gmn = np.degrees(np.arccos(np.clip(
                        np.abs(vp_n[keep])
                        / np.maximum(vmag[keep], 1e-12), 0, 1)))
                    gk = gk_full[keep]
                    for si, sqv in enumerate(SQ_GRID):
                        vts = vtn*np.exp(sqv*gk)
                        h,_,_ = np.histogram2d(
                            np.clip(vts,0.021,5.9), gmn,
                            bins=[VE, GE])
                        p0 = np.maximum(h/max(h.sum(),1), 1e-5)
                        p0 /= p0.sum()
                        for yi, ff in enumerate(FFLY_GRID):
                            wch = min(FC0*SC2[bi], 0.5)
                            wfl = min(ff*SC2[bi], 0.5)
                            wtot = min(wch+wfl, 0.6)
                            mixc = (wch*UB[bi]
                                    + wfl*FB[bi])/(wch+wfl)
                            pp = (1-wtot)*p0 + wtot*mixc
                            C[fi, yi, pi, ki, si, wi] = np.log(pp)
    return C

def eval_pp_nb(p, prj, PLs, UB, FB, cell):
    """pp per NARROW bin at one cell — verbatim pp path."""
    fi, yi, fpm, ki, sqv = cell
    fcm = FCOMP_GRID[fi]; kwv = KW_GRID[ki]; ff = FFLY_GRID[yi]
    smag, vpar, vper = prj
    s_kau = smag/1e3
    pps = []
    for bi in range(NB_BINS):
        b = SBINS[bi]
        idx = np.where((s_kau>=b[0])&(s_kau<b[1]))[0]
        if len(idx) < 500 or len(PLs[bi]) == 0:
            pps.append(None); continue
        vc = 2*np.pi*np.sqrt(p['M_s'][idx]/smag[idx])
        sg0 = PLs[bi][p['pick'][bi][idx] % len(PLs[bi])]/4.74047
        g1_i, g2_i = p['gn1'][idx], p['gn2'][idx]
        sk_i = s_kau[idx]
        gk_full = p['gs'][idx]
        cvp = np.zeros(len(idx)); cvq = np.zeros(len(idx))
        mh_tot = np.zeros(len(idx))
        for k in (1, 2):
            c = p['comp'][k]
            act = c['uc'][idx] < fcm
            mh_tot += act*c['mh'][idx]
            cvp += act*c['w'][idx]*c['wd'][idx,0]
            cvq += act*c['w'][idx]*c['wd'][idx,1]
        boost = np.sqrt(1+mh_tot/p['M_s'][idx])
        vp_n = (vpar[idx] + kwv*cvp)*boost + g1_i*sg0*fpm
        vq_n = (vper[idx] + kwv*cvq)*boost + g2_i*sg0*fpm
        vmag = np.hypot(vp_n, vq_n)
        keep = vmag*4.74047 <= (2.978/np.sqrt(sk_i)
                                + 2.8284*sg0*4.74047)
        vtn = (vmag/vc)[keep]
        gmn = np.degrees(np.arccos(np.clip(
            np.abs(vp_n[keep])/np.maximum(vmag[keep], 1e-12), 0, 1)))
        gk = gk_full[keep]
        vts = vtn*np.exp(sqv*gk)
        h,_,_ = np.histogram2d(np.clip(vts,0.021,5.9), gmn,
                               bins=[VE, GE])
        p0 = np.maximum(h/max(h.sum(),1), 1e-5)
        p0 /= p0.sum()
        wch = min(FC0*SC2[bi], 0.5)
        wfl = min(ff*SC2[bi], 0.5)
        wtot = min(wch+wfl, 0.6)
        mixc = (wch*UB[bi] + wfl*FB[bi])/(wch+wfl)
        pps.append((1-wtot)*p0 + wtot*mixc)
    return pps

def eval_pp_nb_sq(p, prj, PLs, UB, FB, cell):
    """eval_pp_nb with an EXPLICIT sq application in the injection
    (verbatim vts = vtn*exp(sqv*gk) path is already in eval_pp_nb;
    this wrapper only exists to make the co-read call site explicit)."""
    return eval_pp_nb(p, prj, PLs, UB, FB, cell)

def marg_fpm(T):
    """The ARCHIVED marginalization, verbatim: LNPI over fcomp, sum over
    ffly/kw/ws2 -> (fpm, sq) weights -> fpm marginal."""
    lw = logsumexp(T + LNPI.reshape(6, 1, 1, 1, 1, 1), axis=(0, 1, 3, 5))
    wq = np.exp(lw - logsumexp(lw))
    return wq.sum(axis=1)

def S_of(T):
    m = marg_fpm(T)
    return float(np.log(m[3:].sum()) - np.log(m[:3].sum()))

okv = ok
ruwe = np.maximum(_pick('ruwe1', 'RUWE1'),
                  _pick('ruwe2', 'RUWE2'))[okv]
qs_ = np.percentile(ruwe, [25, 50, 75])
QMASKS = [ruwe <= qs_[0],
          (ruwe > qs_[0]) & (ruwe <= qs_[1]),
          (ruwe > qs_[1]) & (ruwe <= qs_[2]),
          ruwe > qs_[2]]
ALLM = np.ones(len(s_d), dtype=bool)
noise_pool = build_stratum(ALLM)[1]
STRATA = [build_stratum(mk) for mk in QMASKS]

ARCH_EFPM = {31: (2.21, 2.17, 2.26, 2.12), 101: (2.13, 2.27, 2.25, 2.30)}
TRUTHS = (1.0, 1.2, 1.5, 1.8, 2.1, 2.4)
R = 64

t0 = time.time()
P("9L-b THE CALIBRATED CONTRAST STATISTIC (correction arc leg 1; "
  "bars pre-committed BEFORE the run; S = ln P(fpm>=2.1) - "
  "ln P(fpm<=1.8); flat posterior S = 0 exactly; NO credence "
  "movement under any letter)")
P("")

g0_ok = g1_ok = True
g2_ok = {}; g3_ok = {}
save = {}
sky_S = {}

for seed in SEEDS:
    pf = build_pop(seed)
    e_f = e_of_x(pf, 1.05, 0.30)
    tab0 = np.ones_like(TAB_S)
    vp0 = vp_c(pf, e_f, tab0)
    o0 = run(pf['a_s'], e_f, pf['psi0'], pf['f_ip'], pf['M_s'],
             pf['uph'], 8, 2500, 5, a0=A0_CAN, tab=tab0,
             lny0=LNY0, dlny=DLNY, vp=vp0)
    prj0 = project(pf, o0)
    C0 = build_logpp_cache(pf, prj0, STRATA[0][1], STRATA[0][2],
                           STRATA[0][3])
    nd = int(STRATA[0][0][0].sum())

    # --- GB9LB-1 reader identity: saved sky tables reproduce the archive
    npz = np.load(f'data/stage9l_tables_{seed}.npz')
    T_nb, LNPI_S = npz['T'], npz['LNPI']
    dl = float(np.max(np.abs(LNPI_S - LNPI)))
    ok_l = dl <= 1e-9
    g1_ok &= ok_l
    P(f"[seed {seed}] GB9LB-1 LNPI identity: max|d| = {dl:.2e} -> "
      f"{'PASS' if ok_l else 'FAIL'}")
    for qi in range(4):
        ef_ = float(np.sum(marg_fpm(T_nb[qi])*FPM_GRID))
        ok_r = abs(ef_ - ARCH_EFPM[seed][qi]) <= 0.01
        g1_ok &= ok_r
        P(f"[seed {seed}] GB9LB-1 reader Q{qi+1}: E[fpm] = {ef_:.3f} "
          f"vs archived {ARCH_EFPM[seed][qi]:.2f} -> "
          f"{'PASS' if ok_r else 'FAIL'}")

    # --- GB9LB-0 cache identity on one synthetic dataset (truth 2.1)
    pps = eval_pp_nb(pf, prj0, STRATA[0][1], STRATA[0][2],
                     STRATA[0][3], (2, 0, 2.1, 1, 0.0))
    rng_id = np.random.default_rng([9, seed, 99])
    draw = rng_id.multinomial(nd, pps[0].ravel()/pps[0].sum())
    synD2 = draw.reshape(NV, NG).astype(float)
    T_dir = eval_block_nb(pf, prj0, [synD2, None, None, None],
                          STRATA[0][1], STRATA[0][2], STRATA[0][3])
    T_cch = np.tensordot(C0, synD2, axes=([6, 7], [0, 1]))
    d0 = float(np.max(np.abs(T_dir - T_cch)))
    ok0 = d0 <= 1e-6
    g0_ok &= ok0
    P(f"[seed {seed}] GB9LB-0 cache identity: max|dT| = {d0:.2e} -> "
      f"{'PASS' if ok0 else 'FAIL'}")
    if not ok0:
        continue

    # --- the calibration ladder
    means, sds, allS = [], [], {}
    for ti, truth in enumerate(TRUTHS):
        pps = eval_pp_nb(pf, prj0, STRATA[0][1], STRATA[0][2],
                         STRATA[0][3], (2, 0, truth, 1, 0.0))
        pr = pps[0].ravel()/pps[0].sum()
        rng_t = np.random.default_rng([9, seed, ti])
        Ss = np.empty(R)
        for r in range(R):
            dr = rng_t.multinomial(nd, pr).reshape(NV, NG).astype(float)
            Ss[r] = S_of(np.tensordot(C0, dr, axes=([6, 7], [0, 1])))
        means.append(Ss.mean()); sds.append(Ss.std(ddof=1))
        allS[truth] = Ss
        P(f"[seed {seed}] calib truth {truth:.1f}: S = "
          f"{Ss.mean():+.3f} +- {Ss.std(ddof=1):.3f} "
          f"(R = {R})  ({(time.time()-t0)/60:.1f} min)")
    means = np.array(means); sds = np.array(sds)
    sig_pool = float(np.sqrt(np.mean(sds**2)))
    se = sds/math.sqrt(R)

    # GB9LB-2 monotonicity (adjacent inversions < 1 pooled SE tolerated)
    se_pool = sig_pool/math.sqrt(R)
    inv = np.diff(means)
    ok2 = bool(np.all(inv > -se_pool))
    g2_ok[seed] = ok2
    P(f"[seed {seed}] GB9LB-2 monotonicity: min adjacent step = "
      f"{inv.min():+.3f} vs -1 SE = {-se_pool:.3f} -> "
      f"{'PASS' if ok2 else 'FAIL'}")
    # GB9LB-3 power
    Dp = means[-1] - means[0]
    ok3 = Dp >= 2*sig_pool
    g3_ok[seed] = bool(ok3)
    P(f"[seed {seed}] GB9LB-3 power: D = S(2.4) - S(1.0) = {Dp:.3f} "
      f"vs 2 sigma_pool = {2*sig_pool:.3f} -> "
      f"{'PASS' if ok3 else 'FAIL'}")

    # --- the sky read (from the SAVED tables)
    Sq = [S_of(T_nb[qi]) for qi in range(4)]
    Sp = S_of(T_nb.sum(axis=0))
    sky_S[seed] = Sq
    P(f"[seed {seed}] SKY S(Q1..Q4) = "
      + ", ".join(f"{x:+.3f}" for x in Sq)
      + f"; pooled = {Sp:+.3f}")
    zrow = [(Sq[0] - means[ti])/sds[ti] for ti in range(len(TRUTHS))]
    P(f"[seed {seed}] PRIMARY Q1 z(truth) = "
      + ", ".join(f"{t:.1f}:{z:+.2f}" for t, z in zip(TRUTHS, zrow)))
    exc = [int(np.sum(allS[t] >= Sq[0])) for t in TRUTHS]
    P(f"[seed {seed}] Q1 empirical exceedance counts (of {R}) = "
      + ", ".join(f"{t:.1f}:{c}" for t, c in zip(TRUTHS, exc)))

    # --- co-read 1: injection nuisance cell at sq = 0.2 (report-only)
    for truth in (1.0, 2.4):
        pps = eval_pp_nb_sq(pf, prj0, STRATA[0][1], STRATA[0][2],
                            STRATA[0][3], (2, 0, truth, 1, 0.2))
        pr = pps[0].ravel()/pps[0].sum()
        rng_t = np.random.default_rng([19, seed, int(truth*10)])
        Ss = np.empty(R)
        for r in range(R):
            dr = rng_t.multinomial(nd, pr).reshape(NV, NG).astype(float)
            Ss[r] = S_of(np.tensordot(C0, dr, axes=([6, 7], [0, 1])))
        P(f"[seed {seed}] co-read sq=0.2 truth {truth:.1f}: S = "
          f"{Ss.mean():+.3f} +- {Ss.std(ddof=1):.3f}; z_sky = "
          f"{(Sq[0]-Ss.mean())/Ss.std(ddof=1):+.2f}")

    # --- co-read 2: model eta = 1.3 (report-only)
    e_alt = e_of_x(pf, 1.3, 0.30)
    vpA = vp_c(pf, e_alt, tab0)
    oA = run(pf['a_s'], e_alt, pf['psi0'], pf['f_ip'], pf['M_s'],
             pf['uph'], 8, 2500, 5, a0=A0_CAN, tab=tab0,
             lny0=LNY0, dlny=DLNY, vp=vpA)
    prjA = project(pf, oA)
    CA = build_logpp_cache(pf, prjA, STRATA[0][1], STRATA[0][2],
                           STRATA[0][3])
    S_sky_alt = S_of(np.tensordot(CA, STRATA[0][0][0],
                                  axes=([6, 7], [0, 1])))
    for truth in (1.0, 2.4):
        pps = eval_pp_nb(pf, prjA, STRATA[0][1], STRATA[0][2],
                         STRATA[0][3], (2, 0, truth, 1, 0.0))
        pr = pps[0].ravel()/pps[0].sum()
        rng_t = np.random.default_rng([29, seed, int(truth*10)])
        Ss = np.empty(R)
        for r in range(R):
            dr = rng_t.multinomial(nd, pr).reshape(NV, NG).astype(float)
            Ss[r] = S_of(np.tensordot(CA, dr, axes=([6, 7], [0, 1])))
        P(f"[seed {seed}] co-read eta=1.3 truth {truth:.1f}: S = "
          f"{Ss.mean():+.3f} +- {Ss.std(ddof=1):.3f}; z_sky(alt) = "
          f"{(S_sky_alt-Ss.mean())/Ss.std(ddof=1):+.2f} "
          f"(S_sky_alt = {S_sky_alt:+.3f})")

    save[seed] = dict(means=means, sds=sds, Sq=np.array(Sq), Sp=Sp,
                      allS=np.array([allS[t] for t in TRUTHS]))
    P("")

np.savez('data/stage9lb_calib.npz',
         truths=np.array(TRUTHS), R=R,
         **{f's{seed}_{k}': v for seed in save
            for k, v in save[seed].items() if k != 'allS'},
         **{f's{seed}_allS': save[seed]['allS'] for seed in save})

if not (g0_ok and g1_ok):
    P("GB9LB-0/1 FAILED - wiring/reader error; STOP; DO NOT QUOTE; "
      "verdict WITHHELD")
elif not (all(g2_ok.values()) and all(g3_ok.values())):
    P("==> 9L-b LETTER (locked grammar): B-POWER-DEAD - the narrow "
      "bin cannot measure fpm at catalog grade (GB9LB-2 "
      + "/".join('PASS' if g2_ok[s] else 'FAIL' for s in SEEDS)
      + ", GB9LB-3 "
      + "/".join('PASS' if g3_ok[s] else 'FAIL' for s in SEEDS)
      + "); the error claim moves ENTIRELY to the joint fits; the "
      "note is withdrawn or reframed onto them. NO credence movement.")
else:
    z10 = {}
    bound = {}
    for seed in SEEDS:
        m, s = save[seed]['means'], save[seed]['sds']
        Sq1 = save[seed]['Sq'][0]
        z = (Sq1 - m)/s
        z10[seed] = z[0]
        b = 0.0
        for ti, t in enumerate(TRUTHS):
            if z[ti] >= 2:
                b = t
        bound[seed] = b
    zmin = min(z10.values())
    if zmin >= 2:
        bq = min(bound.values())
        P(f"==> 9L-b LETTER (locked grammar): B-NOISE-BOUNDED - the "
          f"boost-free bin demands inflation; z(1.0) = "
          + ", ".join(f"{z10[s]:+.2f}" for s in SEEDS)
          + f"; quotable bound: effective inflation > {bq:.1f}x formal "
          "at 2-sigma calibration grade (per population realization; "
          "injection-cell conditional, co-reads shipped). NEVER quote "
          "an E[fpm] point value. NO credence movement (parked).")
    elif zmin < 1:
        P(f"==> 9L-b LETTER (locked grammar): B-NULL-CONSISTENT - the "
          f"narrow bin is consistent with formal errors (z(1.0) = "
          + ", ".join(f"{z10[s]:+.2f}" for s in SEEDS)
          + "); the note's claim is WITHDRAWN; joint-fit language "
          "only; the 9L fork reads toward BOOST-EATEN pending the "
          "review round. NO credence movement (parked).")
    else:
        P(f"==> 9L-b LETTER (locked grammar): B-GRAY - lean-grade "
          f"only (z(1.0) = "
          + ", ".join(f"{z10[s]:+.2f}" for s in SEEDS)
          + "); no bound quoted; the note is withdrawn or reframed "
          "onto the joint fits. NO credence movement (parked).")
P(f"\ndone ({(time.time()-t0)/60:.1f} min)")

with open('data/stage9lb_contrast.txt', 'w') as f:
    f.write("\n".join(L_) + "\n")
print("\nsaved: data/stage9lb_contrast.txt")

# ---------------------------------------------------------------------------
# POST-ROUND-43 NOTE (2026-09-03, appended AFTER the report; non-functional;
# the pre-registered blob is commit 390bd88 -- this comment is not part of it):
# (1) The pooled four-quartile sky S printed above (+2.204/+0.921) is
#     UNCALIBRATED in this stage. ROUND 43 + calcs/round43_gb.py calibrated
#     it: ALSO POWER-DEAD (D 1.15-1.75 vs 2sigma 2.0-2.4), and its sky
#     z(1.0) = +2.4 / +1.8-2.2 (draw-grade) is a truth-uniform artifact
#     that would read as a false B-NOISE-BOUNDED under the z-grammar alone.
#     NEVER read the pooled S as a noise measurement.
# (2) R43 oracle bound (banked): the Neyman-Pearson ceiling for truth 1.0
#     vs 2.4, all nuisances known, is 0.93-1.07 sigma per quartile (Q4
#     slightly exceeds 1; R44-F2) and 1.98 sigma pooled over all 9,950
#     narrow pairs -- the bin, not the statistic, is the limitation.
# ---------------------------------------------------------------------------
