"""
STAGE 3B: hierarchical inference v1.
Joint fit of (alpha_grav, eta_ecc, f_triple) against the full (s-bin x vtilde)
distribution, with the orbit population RE-REALIZED self-consistently at every
grid point (the realization systematic becomes a parameter, not a bias).
Laws: 'simple' and 'BE' QUMOND-solved EFE tables, boost_alpha = 1+alpha*(B-1).
Likelihood: per s-bin multinomial over 40 vtilde bins, noise-convolved with the
data's own per-bin error distributions.
"""
import time, json
import numpy as np
...
for b in SBINS:
    ...
    data_counts.append(c); data_srel.append(srel_d[m])

# ---------------- shared population draws ----------------
...
def lnL_point(o, e_s):
    ...
    for bi, b in enumerate(SBINS):
        ...
        for fi, ft in enumerate(FT_GRID):
            trip = is3_u[idx] < ft
            ...
            p = np.maximum(h/max(h.sum(),1), 1e-5)
            p = p/p.sum()
            out[fi] += np.sum(data_counts[bi]*np.log(p))
    return out

A_GRID = np.array([0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0])
E_GRID = np.array([0.6, 1.0, 1.25, 1.6, 2.0, 2.5, 3.0])
results = {}
for law, TAB in (("simple", TAB_S), ("BE", TAB_B)):
    cube = np.full((len(A_GRID), len(E_GRID), len(FT_GRID)), np.nan)
    t0 = time.time()
    for ai, al in enumerate(A_GRID):
        tab_a = 1.0 + al*(TAB-1.0)
        for ei, eta in enumerate(E_GRID):
            if al == 0.0 and law == "BE" and "simple" in results:
                cube[ai, ei] = results["simple"][ai, ei]; continue
            e_s = e_of(eta)
            ...
            cube[ai, ei] = lnL_point(o, e_s)
    results[law] = cube
    print(f"{law}: grid done in {(time.time()-t0)/60:.1f} min")
    best = np.unravel_index(np.nanargmax(cube), cube.shape)
    print(f"  best: alpha={A_GRID[best[0]]}, eta={E_GRID[best[1]]}, "
          f"f_t={FT_GRID[best[2]]}, lnL={np.nanmax(cube):.1f}")
    prof = np.nanmax(cube, axis=(1,2))
    print("  alpha profile (lnL - max):",
          np.round(prof-np.nanmax(prof), 1).tolist())
    print(f"  Newton rejection: dlnL(alpha_hat vs 0) = "
          f"{np.nanmax(prof)-prof[0]:+.1f}")
np.save('data/hier_cube_simple.npy', results['simple'])
np.save('data/hier_cube_BE.npy', results['BE'])
print("\ncubes saved; grids:", A_GRID.tolist(), E_GRID.tolist(), FT_GRID.tolist())
