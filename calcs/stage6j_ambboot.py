"""
STAGE 6J (O16b): galaxy bootstrap of the ambient-gated lead -- the
pending statistical grade for the rule's galaxy leg.

6C graded F4's -64.2 as -57.4 +/- 38.3 (37/40); 5N graded gm's -40.6 as
-29.3 +/- 53.0 (29/40). Same machinery (40 paired galaxy-resample reps,
dv-ON vertical, lensing noise redraws, rng 53, warm-lite), referent law
chosen by the 6I verdict: 'pgmax' = per-galaxy Chae maxclust gates
(the rule as prescribed) or 'glob' = fiducial global gate s = 0.8681.
Full-fit regression gates: BE vs 5M -12152.49 (<1.5); AMB variant vs
the 6I absolute (parsed from data/stage6i_chaegate.txt, <3.0).
Writes data/stage6j_ambboot.txt.
"""
import csv, glob, math, os, re, sys
import numpy as np
from scipy.optimize import minimize, minimize_scalar

LAW = sys.argv[1] if len(sys.argv) > 1 else 'pgmax'
assert LAW in ('pgmax', 'glob')
...
def m2hv(th, amb, dml, dv, w_g, lens_obs):
    la0, f, s_int, dlt = th
    ...
    nuv = nu_pg(gN/a0, SPT) if amb else nu_be(gN/a0)
    gm_ = gN*nuv
    se2 = sig2 + s_int*s_int
    r = lgobs - np.log10(gm_) - dv[gidx]
    out = np.sum(w_g[gidx]*(r*r/se2 + np.log(se2)))
    lg = l_gbar[lmask] + dlt
    nul = nu_pg(10**lg/a0, S_LENS) if amb else nu_be(10**lg/a0)
    rl = lens_obs[lmask] - (lg + np.log10(nul))
...
def fit_v(amb, w_g, lens_obs, th0=None, dml0=None, dv0=None,
    ...
    for rd in range(max_rounds):
        ...
        bb = None
        for t0 in starts:
            b = minimize(lambda t: m2hv(t, amb, dml, dv, w_g, lens_obs), t0,
                         method='Nelder-Mead',
        ...
        for _ in range(sweeps):
            fac = f*np.exp(dml[gidx])
            gN = g_gas + fac*g_dsk + g_bul
            nuv = nu_pg(gN/10**la0, SPT) if amb else nu_be(gN/10**la0)
            r0 = lgobs - np.log10(gN*nuv)
            ...
            for gi2 in range(NGal):
                ...
                def od(dl):
                    ...
                    nu2 = (nu_pg(gN2/10**la0, SPT[mm]) if amb
                           else nu_be(gN2/10**la0))
                    rr = lgobs[mm] - np.log10(gN2*nu2) - dv[gi2]
                    s2 = sig2[mm] + se2c
                    return np.sum(rr*rr/s2) + dl*dl/(S_ML*S_ML)
                dml[gi2] = minimize_scalar(od, bounds=(-0.7, 0.7),
                                           method='bounded').x
        cur = m2hv(best.x, amb, dml, dv, w_g, lens_obs)
        if prev is not None and abs(prev - cur) < tol:
            prev = cur
            break
        prev = cur
    b = minimize(lambda t: m2hv(t, amb, dml, dv, w_g, lens_obs),
                 list(best.x), method='Nelder-Mead',
                 options=dict(maxiter=4000, xatol=1e-6, fatol=1e-7))
    if b.fun < best.fun: best = b
    return best, dml, dv

# 6I regression target for the chosen law
tgt = None
if os.path.exists('data/stage6i_chaegate.txt'):
    txt = open('data/stage6i_chaegate.txt').read()
    sec = txt.split('vertical-hardened:')[1].split('plain hier:')[0]
    key = 'PGmax' if LAW == 'pgmax' else 'AMBg'
    m = re.search(rf'{key}:\s+(-\d+\.\d+)', sec)
    if m: tgt = float(m.group(1))

ones = np.ones(NGal)
L = [f"STAGE 6J AMB({LAW})-vs-BE dv-ON bootstrap: {kept} galaxies, "
     f"{len(gobs)} points + {int(lmask.sum())} lensing; 40 paired reps; "
     f"gate s: {'per-galaxy Chae maxclust' if LAW == 'pgmax' else 'global'}"
     f" (median {float(np.median(SPT)):.4f}), lensing {S_LENS:.4f}"]

fb, dmlb, dvb = fit_v(False, ones, l_gobs)
fa, dmla, dva = fit_v(True, ones, l_gobs, th0=fb.x, dml0=dmlb, dv0=dvb)
d_full = fa.fun - fb.fun
ok = abs(fb.fun-(-12152.49)) < 1.5 and (tgt is None or abs(fa.fun-tgt) < 3.0)
L.append(f"full-fit regression: BE {fb.fun:.2f} (5M -12152.49), AMB "
         f"{fa.fun:.2f} (6I {tgt}), d = {d_full:+.2f} -> "
...
for k in range(40):
    ...
    vb, _, _ = fit_v(False, w, lo, th0=fb.x, dml0=dmlb, dv0=dvb,
                     tol=0.5, max_rounds=5, sweeps=2)
    va, _, _ = fit_v(True, w, lo, th0=fa.x, dml0=dmla, dv0=dva,
                     tol=0.5, max_rounds=5, sweeps=2)
    ds.append(va.fun - vb.fun)
    if (k+1) % 10 == 0:
        print(f"  rep {k+1}/40: running mean {np.mean(ds):+.1f}", flush=True)
ds = np.array(ds)
L.append(f"bootstrap (40 reps): AMB({LAW}) - BE = {ds.mean():+.2f} +/- "
         f"{ds.std(ddof=1):.2f}  (AMB better in {int((ds<0).sum())}/40)")
L.append(f"percentiles 16/50/84: "
         f"{np.percentile(ds,[16,50,84]).round(1).tolist()}")
L.append("(comparators: 6C F4 - BE = -57.4 +/- 38.3, 37/40; "
         "5N gm - BE = -29.3 +/- 53.0, 29/40)")

out = "\n".join(L)
print(out)
with open('data/stage6j_ambboot.txt', 'w') as f:
    f.write(out+"\n")
print("\nsaved: data/stage6j_ambboot.txt")
