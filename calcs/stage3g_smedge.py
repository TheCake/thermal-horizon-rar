"""
STAGE 3F: hierarchical fit v4 â€” the broadening-aware fit. Stage 3E showed the
...
than any accounted error budget â€” physical decomposition TBD). v4 adds sigma_m
...
Writes data/hier4b_cube_*.npy + data/stage3g_summary.txt.
"""
import time
import numpy as np
...
E_GRID  = np.array([0.8,1.05,1.3,1.55])
SM_GRID = np.array([0.20,0.25,0.30,0.35,0.40,0.45])
FT_GRID = np.array([0.0,0.05,0.10])
FC_GRID = np.array([0.0,0.02,0.10])
SM_CAP = 0.30   # continuity check: matches the v4 grid edge
...
for law, TAB in (("simple", TAB_S), ("BE", TAB_B)):
    ...
    summary += lines
np.save('data/hier4b_cube_simple.npy', results['simple'])
np.save('data/hier4b_cube_BE.npy', results['BE'])
with open('data/stage3g_summary.txt', 'w') as f:
    f.write("STAGE 3G v4b fit (sigma_m EDGE EXTENSION + contamination + eta "
            "prior), grids:\n"
            f"alpha={A_GRID.tolist()}\neta={E_GRID.tolist()}\n"
            f"sigma_m={SM_GRID.tolist()}\nf_t={FT_GRID.tolist()}\n"
            f"f_c0={FC_GRID.tolist()}\nseed 31; cap profile at "
            f"sigma_m<={SM_CAP}\n\n" + "\n".join(summary) + "\n")
print("\nsaved: data/hier4b_cube_*.npy, data/stage3g_summary.txt")
