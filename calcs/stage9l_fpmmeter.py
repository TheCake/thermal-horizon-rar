...
Output: data/stage9l_fpmmeter.txt
"""
import math
...
for seed in SEEDS:
    ...
    np.savez(f'data/stage9l_tables_{seed}.npz', T=T_nb, LNPI=LNPI)
    efpm = {}
    for qi in range(4):
    ...
    # G9L-3 injected-fpm recovery at the Q1 truth cell
    ...
    lw = logsumexp(T_syn + LNPI.reshape(6, 1, 1, 1, 1, 1),
                   axis=(0, 1, 3, 5))
    wq = np.exp(lw - logsumexp(lw))
    rec = float(np.sum(wq.sum(axis=1)*FPM_GRID))
    ok3 = abs(rec - 2.1) <= 0.25
    g3_ok &= ok3
    P(f"[seed {seed}] G9L-3 injected-fpm recovery: E[fpm] = "
      f"{rec:.2f} vs truth 2.1 -> {'PASS' if ok3 else 'FAIL'}  "
...
with open('data/stage9l_fpmmeter.txt', 'w') as f:
    f.write("\n".join(L_) + "\n")
