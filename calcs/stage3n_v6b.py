"""
STAGE 3N: v6b — the four-item queue from Stage 3M in one fit.
  (1) RADIAL-excess e-component replaces the vetoed circular one:
      p(e) = w_rad*[e ~ 0.9+0.095*u] + (1-w_rad)*[power family(eta)].
  (2) Extended fences: f_comp {0,0.1,0.2}, f_c0 {0,0.1,0.2}, f_fly
      {0,0.05,0.1,0.15}.
  (3) f_pm noise-scale nuisance {1.0,1.25,1.5} (Gaia formal PM errors are
      known to be mildly underestimated; applies to the sigv/sqrt2 noise).
  (4) Best-fit 2D residual dump (simple law): z=(n-Np)/sqrt(Np) per cell,
      gamma-marginal residuals, and Newton's worst cells — localizes both the
      anomaly and any remaining misfit (is the gamma~90 arm unfit?).
Joint 2D (vtilde x gamma) likelihood as in stage3m; seed 31; batched by law
via argv. Writes data/hier6b_cube_<law>.npy + appends data/stage3n_summary.txt.
"""
import sys
import time
import numpy as np
        ...
        for fi, fcm in enumerate(FCOMP_GRID):
            ...
            for pi, fpm in enumerate(FPM_GRID):
                ...
                gmn = np.degrees(np.arccos(np.clip(
                    np.abs(vp_n)/np.maximum(vmag,1e-12), 0, 1)))
...
np.save(f'data/hier6b_cube_{LAW}.npy', cube_p)
...
lines = [
    f"{LAW}: {(time.time()-t0)/60:.1f} min (v6b: radial excess + fences + f_pm)",
...
# residual dump at the global best (and Newton's best) — simple batch only
if LAW == "simple":
...
with open('data/stage3n_summary.txt', 'a') as f:
    f.write("\n".join(lines)+"\n")
print(f"saved: data/hier6b_cube_{LAW}.npy; appended data/stage3n_summary.txt")
