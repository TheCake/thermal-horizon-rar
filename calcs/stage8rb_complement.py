"""STAGE 8R-b — THE COMPLEMENT CHANNELS (the gamma-decomposition of
the width demand).  Pre-registered BEFORE any run.

8R's only-sector channels all died (GLOBAL-DEMAND, -31..-42) — but the
census side-condition asks for the COMPLEMENT: smear everything EXCEPT
the flood sector.  8R's gam channel (smear gamma < 45 only) already
measures one complement cut (sparing gamma >= 45 costs 36-42); the
unmeasured cuts are the finer ones.  Channels here:
    uni    all systems                    (identity)
    x60    smear gamma < 60 only          (spare gamma >= 60)
    xperp  smear gamma < 75 only          (spare the band sector)
With 8R's gam (= spare gamma >= 45) these give the top-down
gamma-profile of the kinematic width demand, and xperp is THE
side-condition channel: the band's feeder systems (gamma >= 75,
computed pre-smear and unmoved by it) stay unsmeared, so the
no-companion band flood collapses to its sq=0 value by construction —
the question is purely what the kinematics charge for it.

Gates (the 8R set): G8Rb-1 cube identity = 0.00e+00; G8Rb-2 census
identity (0.10); G8Rb-3 spared-sector fractions printed; G8Rb-4
mask-off wiring (xperp forced-all == uni, bit).
Bars (locked; >= 3/4 law-seeds):
  V1 VIABLE(xperp) iff Dkin >= -2.0 AND pmf(9|mu_band@profiled) >=
     1e-3, no row <= -5  => the reconciling channel EXISTS (the width
     demand spares the flood sector); successor = cube-grade alpha
     re-run under xperp (final-stamp path reopens).
  V2 NEAR-VIABLE iff -5 < Dkin < -2 rows dominate (>= 3/4 in that
     open interval) with pmf9 >= 1e-3 in >= 3/4: the trade is
     quantified, not free — carried as a measurement.
  V3 SECTOR-DEMAND iff Dkin <= -5.0 in >= 3/4: the gamma >= 75 sector
     ITSELF demands width at kill grade => the pincer on the
     multiplicative-smear class CLOSES (no gamma-shaped smear
     reconciles kinematics and census; the width object is NOT a
     per-system multiplicative v-broadening — points additive/noise-
     side or data-side).  Else MIXED-CARRIED.
  x60 is informational (the curve point between gam and xperp).
NO credence movement (measurement round; pre-stated).
Output: data/stage8rb_complement.txt
"""
import re
                ...
                for pi, fpm in enumerate(FPM_GRID):
                    ...
                    for ch in CH_ORDER:
                        ...
                        if fi == 0 and ki == 0 and pi == 0:
                            spared[ch].append(0.0 if msk is None
...
def census_ch(p, o, fcm, kw, fpm, sq, ch):
    smag, vpar, vper = project(p, o)
    s_kau = smag/1e3
    mu, mu_hi = 0.0, 0.0
    for bi, b in enumerate(SBINS):
...
t0 = time.time()
P("8R-b THE COMPLEMENT CHANNELS (pre-reg committed BEFORE any run; "
  "measurement round; NO credence movement)")
P(f"channels = {CH_ORDER} (+ 8R's gam = spare gamma>=45, quoted); "
  f"sq grid = {SQX.tolist()}")
P("")
g1_ok = g2_ok = g4_ok = True
...
for seed in SEEDS:
    ...
    for law, TAB in (('simple', TAB_S), ('BE', TAB_B)):
        ...
        blk, spared = eval_block(pf, o_f)
        sl = np.squeeze(c9[bx[0], bx[1], bx[2]], axis=1)
        dmax = float(np.max(np.abs(blk['uni'][..., :4] - sl)))
        ok1 = dmax <= 1e-9
        g1_ok &= ok1
        P(f"[{law} {seed}] G8Rb-1 cube identity: max|d| = {dmax:.2e} "
          f"-> {'PASS' if ok1 else 'FAIL'}")
        mu_l, mh_l = census_ch(pf, o_f, sh['fcomp'], sh['kw'],
                               sh['fpm'], sh['sq'], 'uni')
        ok2 = abs(mu_l-sh['mub']) <= 0.10 and abs(mh_l-sh['muh']) <= 0.10
        g2_ok &= ok2
        P(f"[{law} {seed}] G8Rb-2 census identity: mu=({mu_l:.2f}, "
          f"{mh_l:.2f}) -> {'PASS' if ok2 else 'FAIL'}")
        P(f"[{law} {seed}] G8Rb-3 spared fractions per bin: x60 = "
          + "/".join(f"{v:.2f}" for v in spared['x60'])
          + ", xperp = " + "/".join(f"{v:.2f}" for v in spared['xperp']))
        # G8Rb-4: xperp with the mask forced all-true == uni (bit)
        smag_, vpar_, vper_ = project(pf, o_f)
        s_k = smag_/1e3
        okp = True
        for bi, b in enumerate(SBINS):
            idx = np.where((s_k>=b[0])&(s_k<b[1]))[0]
            if len(idx) < 500:
                continue
            vc = 2*np.pi*np.sqrt(pf['M_s'][idx]/smag_[idx])
            sg0 = POOLS[bi][pf['pick'][bi][idx] % len(POOLS[bi])]/4.74047
            vp_n = vpar_[idx]*1.0 + pf['gn1'][idx]*sg0*sh['fpm']
            vq_n = vper_[idx]*1.0 + pf['gn2'][idx]*sg0*sh['fpm']
            vmag = np.hypot(vp_n, vq_n)
            keep = vmag*4.74047 <= (2.978/np.sqrt(s_k[idx])
                                    + 2.8284*sg0*4.74047)
            vtn = (vmag/vc)[keep]
            gk = pf['gs'][idx][keep]
            allm = np.ones(int(keep.sum()), dtype=bool)
            okp &= bool(np.array_equal(vtn*np.exp(sh['sq']*gk),
                                       vtn*np.exp(sh['sq']*(gk*allm))))
        g4_ok &= okp
        P(f"[{law} {seed}] G8Rb-4 mask-off wiring: "
        ...
        for ch in CH_ORDER:
            ...
            mu_p, mh_p = census_ch(pf, o_f, fcm_h, kw_h, fpm_h, sq_h, ch)
            edge = []
            ...
            P(f"[{law} {seed}] {ch:<5}: Dkin={dk:+8.2f}; cell "
              f"fcomp={fcm_h:.2f} fpm={fpm_h:.1f} kw={kw_h:.1f} "
              f"sq={sq_h:.1f}; census@prof mu=({mu_p:.2f}, {mh_p:.2f}) "
...
if not (g1_ok and g2_ok and g4_ok):
    P("GATES FAILED (G8Rb-1 " + ('PASS' if g1_ok else 'FAIL')
      + ", G8Rb-2 " + ('PASS' if g2_ok else 'FAIL')
      + ", G8Rb-4 " + ('PASS' if g4_ok else 'FAIL')
      + ") - STOP; DO NOT QUOTE; verdict WITHHELD")
else:
    P("GATES: G8Rb-1 4/4, G8Rb-2 4/4, G8Rb-3 reported, G8Rb-4 4/4 "
      "- ALL PASS")
    P("")
    r = rows['xperp']
    n_v = sum(1 for x in r if (x['dk'] >= -2.0 and x['p9'] >= 1e-3))
    n_n = sum(1 for x in r if (-5.0 < x['dk'] < -2.0))
    n_p9 = sum(1 for x in r if x['p9'] >= 1e-3)
    n_k = sum(1 for x in r if x['dk'] <= -5.0)
    P(f"V-bars xperp: viable-rows {n_v}/4; near-rows {n_n}/4 "
      f"(-5<Dkin<-2); pmf9-rows {n_p9}/4; kill-rows {n_k}/4")
    if n_v >= 3 and n_k == 0:
        P("==> 8R-b VERDICT (locked grammar): VIABLE — the width "
          "demand SPARES the flood sector: smearing everything except "
          "gamma >= 75 keeps the kinematic likelihood and clears the "
          "census band; the reconciling channel EXISTS; successor = "
          "the cube-grade alpha re-run under xperp (the final-stamp "
          "path REOPENS).")
        P("WINNERCH: xperp")
    elif n_n >= 3 and n_p9 >= 3:
        P("==> 8R-b VERDICT (locked grammar): NEAR-VIABLE — sparing "
          "the flood sector costs 2-5 lnL while clearing the band; "
          "the trade is quantified and carried as a measurement (no "
          "successor promotion; the gamma >= 75 sector carries a "
          "small but real width demand).")
        P("WINNERCH: none")
    elif n_k >= 3:
        P("==> 8R-b VERDICT (locked grammar): SECTOR-DEMAND — the "
          "gamma >= 75 sector itself demands width at kill grade: "
          "with 8P and 8R this CLOSES the pincer on the multiplicative "
          "smear class (no gamma-shaped per-system v-broadening "
          "reconciles kinematics and census); the width object is NOT "
          "a multiplicative v-smear — additive/noise-side or "
          "data-side classes take the floor.")
        P("WINNERCH: none")
    else:
        P("==> 8R-b VERDICT (locked grammar): MIXED-CARRIED.")
        P("WINNERCH: none")
    P("    NO credence movement (pre-stated; measurement round).")
P(f"\ndone ({(time.time()-t0)/60:.1f} min)")

with open('data/stage8rb_complement.txt', 'w') as f:
    f.write("\n".join(L_) + "\n")
print("\nsaved: data/stage8rb_complement.txt")
