"""STAGE 8I-a reader — THE WOBBLE-SURVIVAL INSTRUMENT (pre-reg
59a55cf, committed BEFORE any run; measurement round, credence
FROZEN in every branch).

Reads the wsrv cubes (10-dim), G8I-1 arithmetic identity vs the esec
(1.0, 0.95) slice, posteriors on wcut/fcomp/sq/fpm, alpha/dN, the
Dwob analogue, and THE CENSUS FORWARD at the repaired MAP cell
(surv-culled band_mu, 8H convention, observed pair (9, 2)).
Bars: W-DEMANDED / W-REFUSED / W-PARTIAL + CENSUS-REOPENED; the
fifth-move-shaped sentence is pre-scripted DEFER-only.
Output: data/stage8i_read.txt
"""
import os
...
P("8I-a THE WOBBLE-SURVIVAL READ (pre-reg 59a55cf; bars locked "
...
def band_mu(p, o, fcm, kw, fpm, sq, wct):
    """8H band_mu + the 8I survival cull (the pair is absent when the
    summed active wobble amplitude exceeds wcut)."""
    ...
    for bi, b in enumerate(SBINS):
        ...
        for k in (1, 2):
            ...
            wamp += act*c['w'][idx]
        sv = kw*wamp*4.74047 <= wct
        idx = idx[sv]
        cvp, cvq, mh_tot = cvp[sv], cvq[sv], mh_tot[sv]
...
for seed in SEEDS:
    ...
    for law, TAB in (('simple', TAB_S), ('BE', TAB_B)):
        cp = f'data/stage7j_cube_full_wsrv_{seed}_{law}.npy'
        ...
        # G8I-1: the survival-off slice vs the esec slice, arithmetic
        es = (np.load(ep)[:, :, :, 1, 2]
        ...
        ok1 = dq <= 1e-9
        g1_ok &= ok1
        a_id, dn_id, _, po_id = read_any(es, axes_post=(3, 8))
        a_fr, dn_fr, _, po = read_any(cb10, axes_post=(3, 6, 8, 9))
        # Dwob analogue (kw axis 7)
        cks = []
        for ki in (0, 2):
            sl = cb10[:, :, :, :, :, :, :, ki:ki+1]
            cks.append(float(np.nanmax(sl) - np.nanmax(sl[:, :, :, 3:])))
        dwob = cks[1]-cks[0]
        # census forward at the repaired MAP cell + the off comparator
        ...
        kw = KW_GRID[bx[7]]; wct = WCUT_GRID[bx[8]]; sq = SQ_GRID[bx[9]]
        if pop is None:
            pop = build_pop(seed)
        ...
        mu_r, mh_r = band_mu(pop, o_f, fcm, kw, fpm, sq, wct)
        mu_o, mh_o = band_mu(pop, o_f, fcm, kw, fpm, sq, 1e9)
        res[(law, seed)] = dict(
            a_id=a_id, dn_id=dn_id, a_fr=a_fr, dn_fr=dn_fr,
            pwc=po[8], pfc=po[3], pfp=po[6], psq=po[9],
            pfc_id=po_id[3], dwob=dwob,
            cell=(al, eta, wr, fcm, fpm, kw, wct, sq),
            cen=(mu_r, mh_r, jointp(mu_r, mh_r)),
            cen_off=(mu_o, mh_o, jointp(mu_o, mh_o)))
        P(f"[{law} {seed}] G8I-1 max|off-esec| = {dq:.2e} -> "
          f"{'PASS' if ok1 else 'FAIL'}")
        P(f"[{law} {seed}] OFF:  a={a_id:.2f} dN={dn_id:+.1f} "
          f"P(fcomp)={np.round(po_id[3], 2).tolist()}")
        P(f"[{law} {seed}] WSRV: a={a_fr:.2f} dN={dn_fr:+.1f} "
          f"P(wcut)={np.round(po[8], 2).tolist()} "
        ...
        P(f"[{law} {seed}] MAP cell: alpha={al}, eta={eta}, wr={wr}, "
          f"fcomp={fcm}, fpm={fpm}, kw={kw}, wcut={wct}, sq={sq}")
        P(f"[{law} {seed}] CENSUS repaired: mu=({mu_r:.2f}, {mh_r:.2f}) "
          f"jointP={jointp(mu_r, mh_r):.2e} | survival-off: "
...
if pend:
    ...
    P("G8I-1 FAIL - reader wiring suspect; DO NOT QUOTE")
else:
    P("G8I-1 4/4 PASS (survival-off slice = esec (1.0,0.95) slice, "
      "arithmetic)")
    P("")
    # ---- bars -----------------------------------------------------------
    branch = {}
    for law in ('simple', 'BE'):
        pwc = np.mean([res[(law, s)]['pwc'] for s in SEEDS], axis=0)
        pfin = float(pwc[:-1].sum())
        mode_i = int(np.argmax(pwc))
        da = float(np.mean([res[(law, s)]['a_fr']-res[(law, s)]['a_id']
                            for s in SEEDS]))
        br = ('W-DEMANDED' if pfin >= 0.90 else
              'W-REFUSED' if pwc[-1] >= 0.50 else 'W-PARTIAL')
        edge = mode_i == 0 and pwc[0] >= 0.5
        branch[law] = (br, pfin, mode_i, da, edge)
        P(f"BAR [{law}]: P(wcut<inf)={pfin:.2f}, mode="
          f"{WCUT_GRID[mode_i]}, d_alpha={da:+.3f}"
          + (' MATERIAL' if abs(da) > 0.11 else '')
          + (' [EDGE at 0.05]' if edge else '') + f" -> {br}")
    ...
    v = (('W-DEMANDED: the sky wants the survival cap; the '
          'wobble-tail repair is DATA-SUPPORTED')
         if brs == {'W-DEMANDED'} else
         ('W-REFUSED: the sky refuses the repair; the strain stands '
          'unresolved; the census stays confounded')
         if brs == {'W-REFUSED'} else
         'W-PARTIAL/mixed: decomposition reported as-is')
    if reop >= 3:
        v += ('; CENSUS-REOPENED (conditional on 8I-b power) - the '
              '8H demotion lifts pending validation')
    ...
    if fifth:
        v += ('; fifth-move-shaped movement observed - VERDICT '
              'DEFERRED to the powered 8I-b')
    P(f"==> 8I-a VERDICT (locked bars): {v}")
    P("    CREDENCE FROZEN (pre-stated, every branch); 8I-b = the "
      "powered decider.")
P(f"\ndone ({(time.time()-t0)/60:.1f} min)")

with open('data/stage8i_read.txt', 'w') as f:
    f.write("\n".join(L) + "\n")
print("\nsaved: data/stage8i_read.txt")
