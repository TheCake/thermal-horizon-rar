...
AMENDMENT A4 (2026-08-11, logged pre-quote after run 3): run 3 fixed
the integrator (0d PASS 1.85e-3; 0c/0f/G4/G2 PASS; wedge structure
stable across every robustness axis) but 0a/0b/3 still failed -- and
the signature is now diagnostic: drift 1e-5..1e-3 with map values
bouncing +-0.04..0.09 BETWEEN ensembles. That is ensemble-DRAW noise:
the raw MLE on N orbits carries sigma ~ (1+alpha)/sqrt(N) = 0.08 at
N = 600 (A3's "0.008" estimate confused within-ensemble half-window
agreement with between-ensemble scatter -- owned here). The 0b bar
(0.02) is unreachable by the raw estimator at ANY feasible N (~8e4
orbits/cell at 1.4 s/orbit). A4 = the CONTROL-VARIATE estimator (the
7F/6O estimator-amendment precedent, caught by the stage's own gates):
the map is estimated as alpha_i + (alpha_hat_mixed -
alpha_hat_initial) on the SAME orbit set, cancelling the common draw
deviation; chains apply the same subtraction per link (alpha_hat_out -
alpha_hat_in); the G3 row (fresh seed 101) then tests exactly the
between-ensemble reproducibility of THIS estimator at its 0.03 bar.
The thermal-FP gate 0b becomes a pure mixing-deviation test, which is
its pre-registered intent. Bars, gates, letters, wedge rules, step
bars, seeds: unchanged. Run 3 artifacts preserved:
data/stage10o_run3.log + the JSON regenerated by run 4.

AMENDMENT A5 (2026-08-11, logged pre-quote after run 4): run 4 passed
everything except 0b (thermal cell Gamma = 1/3 read 0.9738, residual
-0.0262 vs bar 0.02; the Gamma = 1 thermal cell read 1.0049). A paired
diagnostic probe (scratchpad, non-quotable, disclosed): tau 50 vs 150
on the SAME orbits gives CV 0.9775 vs 0.9781 -- NOT under-mixing; and
the deficit reproduced on an independent seed (-0.022). The residual
is either a small Gamma = 1/3-specific estimator bias (suspected:
draw-vs-window sampling-operator asymmetry -- the idempotence gate,
window-vs-window, agrees to 0.003-0.009) or shared noise at the
measured CV-residual grade (~0.016, G3). The 0b bar at 0.02 is ~1.3
sigma of single-draw noise = a coin-flip gate; the instrument-correct
resolution is REPLICATION: the six gate-bearing cells (alpha_i in
{0.8, 1.0, 1.2} x both Gamma) run R = 6 independent draws and gates
0a/0b are evaluated on the replicate MEANS (SE ~ 0.007 => the
unchanged 0.02 bar becomes a ~3 sigma gate). If a real bias exists,
0b now fails at measured significance and the stage books honestly;
...
for Gam in (1.0 / 3.0, 1.0):
    for a_i in ALPHA_GRID:
        ...
        for _ in range(reps):
            ...
            # A4 control-variate: cancel the common draw deviation
            vals.append(a_i + 0.5 * (a_f1 + a_f2) - a_hat0)
        ...
        MAP_TABLE[(Gam, a_i)] = float(np.mean(vals))
        REP_TABLE[(Gam, a_i)] = vals
        se = (np.std(vals, ddof=1) / np.sqrt(len(vals))) if reps > 1 else float('nan')
        print(f"  Gamma={Gam:.3f} alpha_i={a_i:+.2f} -> alpha_f={MAP_TABLE[(Gam, a_i)]:+.4f}"
              f"  (R={reps}, SE {se:.4f}, last halves {a_f1:+.3f}/{a_f2:+.3f},"
              f" drift99 {drift:.1e}, L1 {l1:.3f})")
        if reps > 1:
            print(f"      replicates: " + ", ".join(f"{v:+.4f}" for v in vals))
...
RESULTS['map_replicates'] = {f"{g:.3f},{a:+.2f}": v
                             for (g, a), v in REP_TABLE.items()}
...
for a_i in (-0.99, 1.32, 2.5):
    ...
    val = a_i
    for k in range(1, 4):
        # A4: per-link control variate (alpha_hat_out - alpha_hat_in)
        a_in = fit_alpha(np.sqrt(np.clip(1 - j**2, 1e-12, 1)))
        p1, p2, (j, w, jz), _ = mix_ensemble(j, w, jz, 1.0)
        a_out = fit_alpha(np.concatenate([p1, p2]))
        val = val + (a_out - a_in)
        chain.append(val)
