...
for Gam in (1.0 / 3.0, 1.0):
    for a_i in ALPHA_GRID:
        ...
        # A4 control-variate: cancel the common ensemble-draw deviation
        MAP_TABLE[(Gam, a_i)] = a_i + 0.5 * (a_f1 + a_f2) - a_hat0
        ...
        print(f"  Gamma={Gam:.3f} alpha_i={a_i:+.2f} -> alpha_f={MAP_TABLE[(Gam, a_i)]:+.4f}"
              f"  (raw {0.5*(a_f1+a_f2):+.3f}, draw {a_hat0:+.3f},"
              f" halves {a_f1:+.3f}/{a_f2:+.3f}, drift99 {drift:.1e}, L1 {l1:.3f})")
