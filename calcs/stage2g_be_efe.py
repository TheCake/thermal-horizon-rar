"""
STAGE 2G: test the Bose-Einstein reading's EFE structure on wide binaries.
BE model: g = g_N,int * (1 + n_BE(x)), x = sqrt(|g_tot|/a0) -- the external
field enters INSIDE the mode energy. Compare against data, Newton, AQUAL-CM.
"""
import time
import numpy as np
...
def boost_be(y, eN):
    x = np.sqrt(np.sqrt(y*y+eN*eN*0+ (np.hypot(y,eN))**0/1)*0 + np.hypot(y,eN))
    # x = sqrt(|quadrature total|/a0); y and eN both in a0 units
    n = np.where(x > 40, 0.0, 1.0/(np.expm1(np.minimum(x,40))))
    return 1.0 + n
...
vpb = vp_consistent(a_s,e_s,M_s,boost_be,A0_CAN)
pops['BE-EFE'] = run(a_s,e_s,psi0,f_ip,M_s,uph,10,3000,4,a0=A0_CAN,vp=vpb)
...
print(f"\n{'bin [kAU]':>10} {'DATA':>7} {'Newton':>8} {'AQUAL-CM':>9} {'BE-EFE':>8}")
for i,b in enumerate(BINS):
    print(f"{str(b):>10} {DATA[i]:>7.3f} {profs['newton'][i]:>8.3f} "
          f"{profs['aqual-CM'][i]:>9.3f} {profs['BE-EFE'][i]:>8.3f}")
for k in profs:
