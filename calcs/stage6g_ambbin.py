"""
STAGE 6G (O16 binary leg): the ambient-gated bath on the wide binaries.

6E's rule at the binaries' ambient (each table's gate computed from its
own e_N: g = [n_BE(sqrt(e_N/a0))/(1+n_BE)]^2; at e_N = 1.2, g = 0.112)
gives a function within 0.3% of the pure occupation law at the
transition (nu(1) = 1.577) with tail p = 0.53. PRE-REGISTERED
prediction (6E): mean DlnL vs p050 within ~ -2, interior amplitudes at
BE grade, a0 translation ~ +2 sigma. This is the near-guaranteed leg of
a post-hoc rule (flagged); its value is quantitative: HOW close, and
whether kappa/a0 land BE-grade.
Gates: spherical identity per table; exact-count patch asserts.
Writes data/stage6g_summary.txt + data/stage6g_verdict.txt.
"""
...
for eN, tag in ((1.0, 'g1p0'), (1.2, 'g1p2'), (1.4, 'g1p4')):
    ...
    nu = make_amb(g)
    b = solve(nu, eN)
    np.save(f'data/efe_boost_amb_{tag}.npy', np.stack([y, b]))
    msg = f"table data/efe_boost_amb_{tag}.npy (eN={eN}, g={g:.4f})"
    if eN == 1.2:
        b_iso = solve(nu, 0.0)
...
NEW_OUT = "with open('data/stage6g_summary.txt', 'a') as f:"
assert src.count(OLD_OUT) == 1
src = src.replace(OLD_OUT, NEW_OUT)

have = os.path.exists('data/stage6g_summary.txt') and \
       open('data/stage6g_summary.txt').read().count(' amb: ') >= 6
if not have:
    if os.path.exists('data/stage6g_summary.txt'):
        os.remove('data/stage6g_summary.txt')
    SEEDS6 = ['31', '101', '202', '303', '404', '505']
    ns2 = {'__name__': '__main__',
           'LAMPATH': 'data/efe_boost_amb_g1p2.npy', 'LAMLAW': 'amb'}
    sys.argv = ['stage6g', '1p2'] + SEEDS6
    print("\n===== amb (ambient-gated) on binaries =====", flush=True)
    exec(compile(src, 'stage3p_patched_6g_amb', 'exec'), ns2)
...
for p in ('data/stage5k_summary.txt', 'data/stage5o_summary.txt',
          'data/stage6g_summary.txt'):
    REC.update(parse(p))
SEEDS = [31, 101, 202, 303, 404, 505]
ds = np.array([REC[('amb', s)]['lnl'] - REC[('p050', s)]['lnl']
               for s in SEEDS])
ah = np.array([REC[('amb', s)]['a'] for s in SEEDS])
n_int = sum(REC[('amb', s)]['interior'] for s in SEEDS)
L = ["STAGE 6G: the ambient-gated bath on the binaries", ""]
L.append("per-seed lnL - lnL(p050): " + " ".join(f"{d:+.2f}" for d in ds))
L.append(f"  mean {ds.mean():+.2f} +- {ds.std(ddof=1)/np.sqrt(6):.2f} SE; "
         f"amb better in {int((ds > 0).sum())}/6; interior {n_int}/6; "
         f"a_hat {ah.mean():.3f} +- {ah.std(ddof=1)/np.sqrt(6):.3f}")
L.append("  [6E pre-registered: within ~ -2 of p050. comparators: "
         "gm -8.50, rb2 -5.47, rb4 -6.96, dwf -9.64]")
...
kam = float(np.mean(kappa('amb', lnyd)))
okk = abs(kbe - 0.916) < 0.01
m = float(ah.mean())
stot = math.hypot(float(ah.std(ddof=1)/np.sqrt(6)), 0.11)
a0b = 1.2e-10*m**(1.0/kam)
sa0b = a0b*stot/(m*kam)
tp = 2.998e8*(67.4*1e3/3.0857e22)/(2*math.pi)
pull = (a0b - tp)/math.hypot(sa0b, tp*0.5/67.4)
L.append(f"kappa: be {kbe:+.3f} (vs 4V +0.916 -> "
         f"{'PASS' if okk else 'FAIL'}), amb {kam:+.3f}")
L.append(f"a0: alpha {m:.3f}+-{stot:.3f}, kappa {kam:+.3f} -> "
         f"a0 = {a0b*1e10:.2f}+-{sa0b*1e10:.2f}  pull {pull:+.1f} sigma  "
         f"[BE +1.9; 6E pre-registered ~ +2]")

out = "\n".join(L)
print("\n" + out)
with open('data/stage6g_verdict.txt', 'w') as f:
    f.write(out + "\n")
print("\nSTAGE 6G done -> data/stage6g_verdict.txt")
