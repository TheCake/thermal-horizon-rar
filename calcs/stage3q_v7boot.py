...
def logp_point(o):
    """(FCOMP, FC0, FFLY, 4, NV, NG) log-probabilities"""
    ...
    s_kau = smag/1e3
    out = np.full((len(FCOMP_GRID), len(FC0_GRID), len(FFLY_GRID),
                   4, NV, NG), np.log(1/(NV*NG)))
        ...
        for fi, fcm in enumerate(FCOMP_GRID):
            ...
            for ci, fc in enumerate(FC0_GRID):
                for yi, ff in enumerate(FFLY_GRID):
                    ...
                    out[fi, ci, yi, bi] = np.log((1-wtot)*p0 + wtot*mixc)
    return out
...
for law, TAB in (("simple", TAB_S), ("BE", TAB_B)):
    lp = np.full((len(A_GRID), len(E_GRID), len(WR_GRID),
    ...
    for ai, al in enumerate(A_GRID):
        ...
        for ei, eta in enumerate(E_GRID):
            for wi, wr in enumerate(WR_GRID):
                if al == 0.0 and (ei, wi) in newt:
                    lp[ai, ei, wi] = newt[(ei, wi)]; continue
                e_s = e_of(eta, wr)
                ...
                lp[ai, ei, wi] = logp_point(o)
                if al == 0.0: newt[(ei, wi)] = lp[ai, ei, wi]
    logp[law] = lp
    print(f"{law} grid done, {(time.time()-t0)/60:.1f} min")
...
for k in range(NBOOT):
    ...
    sb, vb, gb = s_d[pk], vt_d[pk], gam_d[pk]
    counts = np.zeros((4, NV, NG))
    for bi, b in enumerate(SBINS):
        m = (sb>=b[0])&(sb<b[1])
        counts[bi],_,_ = np.histogram2d(np.clip(vb[m],0.021,5.9), gb[m],
                                        bins=[VE, GE])
    for law in logp:
        lnl = np.tensordot(logp[law], counts, axes=([6,7,8],[0,1,2]))
        lnl += prior[None,:,None,None,None,None]
        prof = np.nanmax(lnl, axis=(1,2,3,4,5))
...
for law in res:
    ...
    L += [f"{law}: boot alpha_hat = {ah.mean():.3f} +/- {ah.std(ddof=1):.3f} "
          f"(16-84%: [{np.percentile(ah,16):.2f}, {np.percentile(ah,84):.2f}]); "
          f"interior {res[law]['interior']}/{NBOOT}",
          f"{law}: boot Newton dlnL = {dn.mean():.1f} +/- {dn.std(ddof=1):.1f} "
          f"(min {dn.min():.1f})",
