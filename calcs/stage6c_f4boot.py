"""
STAGE 6C (O15c): galaxy bootstrap of the F4 lead -- is the largest
vertical-hardened advantage the program has seen (-64.2, 6A) a
detection or a lean?

5N graded gm's -40.6 as -29 +/- 53 (29/40) -- a lean. F4's lead is 1.6x
larger; the same 40 paired galaxy-resample reps (5N machinery verbatim:
delta_d NM-profiled + delta_v closed-form, lensing noise redraws,
warm-started adaptive-lite) grade it on the same yardstick.
Writes data/stage6c_f4boot.txt.
"""
import glob, math, os
import numpy as np
...
ones = np.ones(NGal)
L = [f"STAGE 6C F4-vs-BE dv-ON bootstrap: {kept} galaxies, "
     f"{len(gobs)} points + {int(lmask.sum())} lensing; 40 paired reps"]

fb, dmlb, dvb = fit_v(nu_be, ones, l_gobs)
ff, dmlf, dvf = fit_v(nu_f4, ones, l_gobs)
d_full = ff.fun - fb.fun
ok = abs(fb.fun-(-12152.49)) < 1.5 and abs(ff.fun-(-12216.70)) < 3.0
L.append(f"full-fit regression: BE {fb.fun:.2f} (5M -12152.49), F4 "
         f"{ff.fun:.2f} (6A -12216.70), d = {d_full:+.2f} -> "
...
for k in range(40):
    ...
    vf, _, _ = fit_v(nu_f4, w, lo, th0=ff.x, dml0=dmlf, dv0=dvf,
                     tol=0.5, max_rounds=5, sweeps=2)
    ds.append(vf.fun - vb.fun)
    if (k+1) % 10 == 0:
        print(f"  rep {k+1}/40: running mean {np.mean(ds):+.1f}", flush=True)
ds = np.array(ds)
L.append(f"bootstrap (40 reps): F4 - BE = {ds.mean():+.2f} +/- "
         f"{ds.std(ddof=1):.2f}  (F4 better in {int((ds<0).sum())}/40)")
L.append(f"percentiles 16/50/84: "
         f"{np.percentile(ds,[16,50,84]).round(1).tolist()}")
L.append("(5N comparator: gm - BE = -29.3 +/- 53.0, 29/40)")

out = "\n".join(L)
print(out)
with open('data/stage6c_f4boot.txt', 'w') as f:
    f.write(out+"\n")
print("\nsaved: data/stage6c_f4boot.txt")
