...
4. **A finite, modest mode count.** N ~ 20–60 effective modes per measurement, realized point-level rather than galaxy-level (§4.6).
...
The first rung of that refit was then executed: a hierarchical fit in which every galaxy receives a free disk-M/L offset (0.1-dex lognormal prior) profiled jointly with the globals, the scatter models recompeting on what remains. **The thermal-direction trend survives**: oscillator-plus-floor beats constant by Δ(−2lnL) = 23 for one parameter on the reduced floor (0.059 dex), with N̂ rising to ≈ 63 — the unmarginalized N ≈ 21 was floor-limited, and we quote the constraint as **N ~ 20–60, nuisance-depth-dependent**. No galaxy-level draw signature appears (the fitted offsets are uncorrelated with each galaxy's acceleration depth, r = −0.02), disfavoring a bath realized per galaxy rather than per point. Two disclosures: the marginalized free-bin shape becomes V-shaped, its Newtonian-side rise sitting exactly where *bulge* mass-to-light scatter — which we did not marginalize — would surface; and the injection gate recovered synthetic per-galaxy offsets at r = 0.74 against a 0.8 target, so cross-model comparisons that re-optimize the offsets — in particular a local-versus-global mode contest via annulus-area scaling, which showed a large but gate-compromised preference — are reported as unresolved rather than measured. The Desmond-(2023)-grade full hierarchy (bulge, distance, inclination) is the decisive instrument and is the program's declared next step on this channel. [`calcs/stage4u_mlmarg.py`]
...
| mode count N (§4.6) | 20–60 (constraint) | — | — |
...
6. **The bath's second moment** (§4.6): the fluctuation channel is new and cheap — the M/L-marginalized scatter-shape fit either isolates the oscillator's monotone term from the transition bump or dissolves the trend into stellar-population scatter; either outcome is informative, and improved distance/inclination errors sharpen N̂ directly.
