...
*Draft 0.6 (2026-08-12): ROUND 40 adopted. That round labelled the two velocity conventions of the perpendicular census and added the band expectation under the fitted absorber budget, replaced the noise-legitimacy argument of Section 4.1 with its quality-stratified form, added the model-light amplitude column to Table 3, defined and attributed the physical error envelope, corrected the model population size, harmonized the allowed-amplitude statement across the paper, and applied twenty-four smaller corrections. Draft 0.5 (2026-08-06) applied the preceding referee round. Sentence pass: mean 23.7 words, maximum 49. References inherit the 2026-07 verified list; the three entries new to this paper (El-Badry & Rix 2018; Lindegren et al. 2021; Tokovinin 2014) were ADS-verified 2026-08-06. Figures are produced by calcs/paper1_figures.py under provenance gates (data/paper1_figs.txt).*

*Acknowledgments: the computational analysis, literature verification, and manuscript drafting were performed in collaboration with Claude (Anthropic). The full chronological program record, including all logged corrections, is in the project repository (Appendix B). A companion paper analyzes the rotation-curve and coefficient-level regime of the same program; the calibration measurement of Section 4 is also reported on its own in a short observational note (Section 4.1).*

## Abstract

Conflicting conclusions have been reported on whether wide stellar binaries show the velocity excess predicted by modified-dynamics theories below a₀. We analyze 14,071 Gaia EDR3 wide pairs with a forward-modeled joint likelihood over the normalized relative velocity and the velocity-separation angle, marginalizing eccentricities, hidden companions, contamination, selection, and measurement noise. First, a calibration measurement independent of gravity: at separations of 0.2–2 kAU, where no candidate theory predicts a measurable boost, the pair-level velocity errors are at least 2.1 times the formal Gaia uncertainties, independent of RUWE; the single-star quality flag does not certify two-body velocity budgets. Second, a methodological result: without the direction channel, velocity-only fits of these data report a spurious boost of roughly half the galactic calibration, which the direction data veto. Accounting for this, for sample construction, and for noise treatment, the three analyses we re-measure become arithmetically compatible. Third, the gravity result: after stratifying by astrometric quality, the cleanest strata exclude a boost amplitude α ≥ 0.5 (relative to the galactic calibration) at every anchoring of the measured companion-rate prior. Amplitudes up to 0.3–0.5 remain allowed, a mild interior preference for 0.1–0.3 appears only at the low-conversion edge of that prior, and α = 0 is not excluded. A census of perpendicular-moving pairs holds nine above the Newtonian velocity bound with only two beyond the boosted edge; no tested model reproduces that pair of counts, and the census is released. Gaia DR4 epoch astrometry will decide both the error model and the limit.
...
Second, the eccentricity sector is externally validated. The fitted near-parabolic weight, w_rad = 0.20, was selected in every fit variation tried (more than thirty), and matches the e > 0.9 mass fraction of 20.4–21.7% implied by the independently measured superthermal eccentricity law of Hwang, Ting & Zakamska (2022) on the same catalog. Freezing w_rad to their value moves the fitted boost by less than 0.01, so the conclusions do not lean on the internal fit of this sector. A separate analysis in the program record (Appendix B) addresses the objection that a present-day eccentricity distribution need not be the one the orbits were born with. Secular phase mixing under the Galactic tide cannot manufacture a superthermal distribution: over 10 Gyr, mixing from near-circular initial conditions leaves the eccentricity power-law index near −0.90, deeply subthermal, and the observed superthermal law is dynamically stable over the same interval. The measured law is therefore primordial in the sense the forward model requires, and the mixed map agrees with the widest separation bin of Hwang et al. to 0.1σ.
...
The narrowest separation bin, 0.2–2 kAU, is effectively Newtonian for every candidate law: the model boost of the bin's median velocity is 0.3–0.8% across the full function and amplitude grid (a gate on this premise excluded the adjacent 2–6 kAU bin, whose model boost reaches 5–8%, from the calibrator). Whatever noise inflation the kinematics of this bin demand is therefore honest instrument noise rather than absorbed signal. Fitting the per-stratum noise parameters on this bin alone, with the boost fixed off, returns an expected proper-motion error inflation of E[f_pm] = 2.21 and 2.13 in the cleanest RUWE quartile for the two population realizations. An injection test recovers a known inflation of 2.1 at 2.12 and 2.06. The eccentricity sector is held at a fixed cell (index η = 1.05, near-parabolic weight w_rad = 0.30) rather than fitted, because the calibrator is not powered to separate the two. A co-read at η = 1.3 returns 2.28 and 2.21, so the eccentricity assumption moves the answer by less than 0.1.

Two properties of this measurement matter downstream. It is flat across astrometric quality: splitting the sample into RUWE quartiles gives E[f_pm] between 2.12 and 2.30 in every quartile, for both realizations. And it is a lower bound: a substantial fraction of the posterior mass sits at the top of the inflation grid, so we quote the result as pair-level velocity errors of at least 2.1 times the formal Gaia uncertainties at 0.2–2 kAU. A pre-registered control bounds the mass-model objection. Re-measuring the meter under global mass rescalings of ×0.80 and ×1.25, tilt deformations of ±10–14% across the main sequence, and a tightened main-sequence window leaves the expectation never below 1.77 times formal. The control missed its own robustness threshold by 0.03 in one cell of that family, so we report it as bounding the objection rather than closing it. At its floor the inflation is still substantial, and it is not an artifact of the photometric mass table.

We believe this number is useful to any analysis of the catalog, independent of the gravity question. The RUWE statistic certifies the single-star five-parameter astrometric solutions; it evidently does not certify the error budget of the relative velocity of a resolved pair, which is sensitive to error channels (local systematics, covariances between nearby sources, scan-geometry effects) that single-star quality cuts do not address. The same inflation is demanded independently by the quality-stratified joint fit over all separations, whose cleanest-quartile noise posterior is E[f_pm] = 1.97–2.25. That agreement, between a fit that can see the boost and a meter that cannot, is what makes the subtraction in Section 6 legitimate rather than circular. The unstratified joint fit is a different matter: it prefers the top of whatever inflation grid it is given, which is the residual-shape object of Section 4.2 rather than a competing measurement of the noise level.

This calibration measurement is also reported on its own, in a short observational note submitted separately. The two manuscripts share this result and no other, and the note carries no gravity fit.

### 4.2 The residual shape question

One aspect of the noise model remains genuinely open. When the inflation grid is extended to 3.0 times formal, the full-sample posterior migrates toward the new edge in both interpolating families, ceding about 8 log-likelihood units and up to 0.06 of the fitted amplitude. No injected population reproduces this behavior: mock skies carrying the measured companion population, the demanded width channel, and the measured inflation put less than 2% posterior mass at the extended edge, against 54–97% on the real sky. The sky therefore contains a residual velocity-width structure that scale inflation of Gaussian errors does not capture. The preferred value is also unphysical as an error scale. Lindegren et al. (2021) document that EDR3 formal uncertainties underestimate single-star astrometric errors by magnitude-dependent factors of roughly 1.05 to 1.4, so a factor of 3.0 is about twice the published ceiling, and the posterior's migration to it is a statement about model shape rather than about Gaia. Two pre-registered alternative shapes (an error-independent velocity floor and a heavy-tail fraction) failed their acceptance bars. Locating diagnostics place the residual in the mid range of the velocity distribution and the radially oriented column of the inner separation bins, not in any tail. Whether this is non-Gaussian error shape or unmodeled population structure is the principal attribution question that Gaia DR4 epoch astrometry can settle, and the fitted amplitudes below are quoted with the sensitivity band this ambiguity implies.
...
|---|---|---|
| full sample, physical Gaia error envelope (f_pm ≤ 1.8; Lindegren et al. 2021) | 0.80 / 0.80 | +35.2 / +32.3 |
| full sample, operative (noise grid to 3.0) | 0.67–0.74 | +14.5 to +23.8 |
| clean strata (Q1–Q3), measured prior | up to 0.3–0.5 allowed, interior preference 0.1–0.3; α ≥ 0.5 excluded | +1.5 to +6.5 at the peak (3 of 4 reads) |
...
We state the implication for the field plainly. On this catalog, a scalar-velocity analysis with standard error modeling can manufacture half a galactic-calibration boost from the width budget, and the direction data are what catch it (Figure 4). Analyses on either side of the published disagreement are exposed to this degeneracy to the extent that they fit velocity distributions with the eccentricity sector, the companion sector, or the noise model free.

![Figure 4](figs/fig6_phantom.png)

*Figure 4. Fitted amplitudes in the four analysis configurations, for both families and both realizations. In the width-free configuration occupied by published scalar-velocity pipelines (left pair), velocity-only fitting reports α ≈ 0.5 which the joint two-dimensional fit vetoes toward zero. With the width channel active (right pair), the two channels agree and the amplitude is channel-robust.*

## 8. The perpendicular census

A bound Newtonian pair can never exceed ṽ = √2, at any eccentricity or phase. This bound and the discriminating power of the band above it are the field's founding observables (Pittordis & Sutherland 2018, 2023; Banik & Zhao 2018). The reason they have not been decisive is contamination: in the full sample the band above √2 is dominated by flybys and multiples, to the point that Banik (2019) proposes the population as a contamination diagnostic. Our contribution is the column of the (ṽ, γ) plane where that degeneracy breaks. Among wide pairs (s ≥ 6 kAU) moving nearly perpendicular to their separation vector (γ ≥ 75°), flyby geometry is disfavored, the direction channel has independently fixed the eccentricity mixture, and a boosted law raises the ceiling to √(2B) ≈ 1.65 at the galactic calibration.

The operative statistic of this section is a pair of counts rather than a count. Under the perspective-corrected convention, nine pairs occupy the Newtonian-forbidden band [1.414, 1.67), two sit in the overshoot region [1.67, 2.2), and none lie beyond.

Two model references must be kept apart, because they use different noise conventions. The first is the formal-noise reference. Were the pair velocity errors the formal Gaia values, a true Newtonian edge would leak about 0.9 pairs into the band, and the Poisson probability of nine or more at that expectation is 5 × 10⁻⁷ (the uncorrected convention places eleven pairs in the band, at 3.8 × 10⁻⁹). A boosted edge at 1.65 with the same formal errors expects 11.6 in the band and 0.5 beyond it, which puts the observed nine at P = 0.28 and the observed two at P = 0.090. That is a comfortable description of the data and a poor one of a Newtonian sky, but the formal level is not the measured noise level, so this comparison is a reference point and not a result.

The second reference is the fitted absorber budget of Section 6, evaluated at the noise inflation and per-system width the joint fit prefers. There the expected band occupancy is 26.2 and 26.4 in the two population realizations against the nine observed, so the sky holds roughly three times fewer band pairs than the fitted Newtonian model wants; the probability of nine or fewer at μ = 26.23 is 9.8 × 10⁻⁵. The band count on its own therefore carries no Newton-rejection power under the landed absorbers, and we retire it as a standalone statistic. The 5 × 10⁻⁷ above belongs to the formal-noise convention and must not be read as a rejection of the fitted model.

What survives the change of convention is the joint occupancy. The same fitted Newtonian configuration that floods the band also floods the region beyond the boosted edge. It expects 17 pairs there at the fitted cell, and up to 32 across the tested Newtonian variants, against the two observed, with the probability of two or fewer at most 7 × 10⁻⁶. Boosted configurations with inflated noise flood it as well. No configuration we searched reproduces nine in the band together with a termination at two; the smallest joint probability any of them reaches is about 9 × 10⁻⁵. One cannot simultaneously hold the noise needed to fill the band and the sharpness of the observed termination, and it is the pair of counts, not either count alone, that no model in this analysis accounts for.

The census survived a dedicated adversarial campaign. Heavy-tailed error manufacture fails to reproduce it across the tested class, which runs to contaminated fractions of 50% at a kurtosis parameter of 4; over that whole range the expected band count never exceeds 2.7 against the nine observed. Repairs aimed specifically at each flank (bulk-variance flooding of the band; the measured subsystem period law aimed at the cliff) are each excluded, the latter refused by the kinematics. Search-based caveats are stated: these are exclusions over the fitted model family and its posterior neighborhood, not over all conceivable error models. At the object level, all nine band pairs are clean in the Gaia non-single-star tables, with no orbital, acceleration, or spectroscopic-trend solutions on either component. The coverage behind that statement is partial. Seven of the nine pairs carry radial-velocity or astrometrically informative coverage and two do not, and two of the seven covered pairs (at 34.2 and 44.4 kAU) carry a radial-velocity variability flag, a mild excess over the wide-sample rate at P ≈ 0.09; the census statistic survives dropping both. An external spectroscopic-archive search reached three of the twenty-two component stars of the band and overshoot pairs, and exactly one band pair received a decade-baseline classification-grade read, which came back quiet. The astrometry of the band pairs is better than the sample average (median maximum RUWE 1.12 in the corrected nine-pair census, 1.06 in the uncorrected eleven-pair one, against 1.28 for the low-angle continuum at the same velocities). Their chance-alignment probabilities have a median of 7 × 10⁻⁴ and span 7 × 10⁻⁶ to 6 × 10⁻³, summing to an expectation of 0.018 chance alignments among the nine. Field-star flybys at closest approach would arrive at normalized velocities of order 100, not 1.5, and would have no ceiling. The low-angle unbound continuum indeed runs smoothly past ṽ = 3, which also demonstrates that the catalog's selection is not the cliff.
...
![Figure 5](figs/fig4_census.png)

*Figure 5. The perpendicular column (s ≥ 6 kAU, γ ≥ 75°): the observed ṽ histogram with the Newtonian-forbidden band and the overshoot region shaded, and the boosted escape edge at 1.65 marked. The inset lists the expected (band, overshoot) occupancies for the three model worlds; none reproduces the observed pair (9, 2).*
...
![Figure 6](figs/fig5_medians.png)

*Figure 6. Median-ratio statistics against forward-modeled expectations. Top group: this work's anchor ratio (with bootstrap interval) against the fitted Newtonian and boosted configurations, which bracket it. Bottom group: the same comparison on the Cookson et al. selection, where the observed step is flat and the model separation is below one sigma.*
...
5. After quality stratification, the cleanest strata exclude boost amplitudes α ≥ 0.5 at every anchoring of the measured companion prior. Amplitudes up to 0.3–0.5 remain allowed, with a mild interior preference for 0.1–0.3 that appears at the low-conversion edge of the prior and dissolves toward zero at mid and high conversion. The Newtonian value α = 0 is not excluded.
6. A model-light median excess of 0.035–0.045 over the Newton-plus-nuisances expectation persists, and the perpendicular column holds nine pairs in the Newtonian-forbidden velocity band with only two beyond the boosted escape edge. That pair of counts, rather than either count alone, is what no tested error or contaminant model reproduces. Both results remain open at small statistical weight.
7. The three published analyses we re-measure on this catalog, spanning the strongest detection and two of the nulls, are arithmetically compatible within this accounting; the disagreement lives in sample construction and the shared error model, not in the sky. A fourth null rests on comparably aggressive cleaning whose sensitivity we have not re-measured.
...
- A boost-corroboration reading of a quality-quartile median ratio was withdrawn when external review showed the statistic does not control separation-dependent noise. The measured values stand and are printed in Table 3; only the corroboration reading was withdrawn.
...
- the quality stratification and the amplitude ladder: stage8w_strata.py, stage8z_dose.py, stage9a_stratalpha.py, stage9d_q4robust.py, stage9e_mlquality.py (the model-light column of Table 3), stage9f_stratmarg.py, stage9i_finealpha.py, stage9j_stdext.py
- the eccentricity secular-mixing calculation of Section 3.3: stage10o_p15erode.py (with stage10o_worker.py)
...
- the figures of this paper: paper1_figures.py (provenance dump data/paper1_figs.txt). The figure files retain their generation-order names, so Figures 4, 5 and 6 of the text are figs/fig6_phantom, figs/fig4_census and figs/fig5_medians respectively

Large datasets are re-fetched by documented URLs (El-Badry, Rix & Heintz 2021 catalog via Zenodo).

## References

(Inherited from the program's 2026-07 INSPIRE-verified list; the three entries new to this paper were verified against ADS on 2026-08-06. Preprint identifiers are given where the verified list carries them.)
...
- Chae, K.-H. 2023, ApJ 952, 128 (arXiv:2305.04613); 2024, ApJ 972, 186 (arXiv:2402.05720)
...
- Milgrom, M. 2010, MNRAS 403, 886 (arXiv:0911.5464)
- Pecaut, M. J., & Mamajek, E. E. 2013, ApJS 208, 9 (arXiv:1307.2657)
- Pittordis, C., & Sutherland, W. 2018, MNRAS 480, 1778 (arXiv:1711.10867); 2023, OJAp (arXiv:2205.02846)
