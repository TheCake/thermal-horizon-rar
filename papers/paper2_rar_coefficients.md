...
A first direct fit of a₀(z) has since appeared at the other end of the lever arm, and it moves this falsifier from untested to constrained. Vărăşteanu et al. (2026) measure the RAR for 130 purely HI-selected galaxies with resolved kinematics out to z ≈ 0.09, and fit the linear form a(z) = a₀ + a₁z. The evolution claim itself originates one paper earlier in the same series, which reported the first tentative evidence for redshift evolution in the acceleration scale from 19 galaxies (Vărăşteanu et al. 2025). Over their redshift window the horizon lock predicts a₁ = +0.52 × 10⁻¹⁰ m s⁻² per unit redshift. That value is the secant of cH(z)/2π for a flat ΛCDM background with H₀ = 70 km s⁻¹ Mpc⁻¹ and Ω_m = 0.3; the derivative at z = 0 is +0.49 in the same units, and the two bracket the linearization ambiguity. Their pure HI-selected sample alone gives a₁ = (−1.60 ± 2.33) × 10⁻¹⁰ m s⁻², which they read as consistent with no evolution. That band contains the no-evolution value at 0.69σ and the lock's predicted slope at 0.91σ, separating the two hypotheses by 0.22σ of its own width. Adding SPARC as a z ≈ 0 anchor, their combined fit returns a₁ = (5.23 ± 1.05) × 10⁻¹⁰ m s⁻², which they label a formal 5.0σ preference for a rising scale; at face value that sits 4.5σ above the lock.

The arbitrating unknown is the zero-point calibration between the two samples, and the authors identify it themselves. Their anchored slope implies a rise of 0.47 × 10⁻¹⁰ m s⁻² across their own redshift range, while the intercepts of their two fits differ by 0.39 × 10⁻¹⁰ m s⁻² (1.54 against 1.15). A slope of 4.33 × 10⁻¹⁰ m s⁻² would reproduce that intercept gap with no evolution at all, and their anchored value sits 0.85σ from it. Their own text reports that the two fits differ at the 2.5σ level, attributes that difference largely to the selection of the low-redshift anchor, and warns that combining samples with different selection functions and modelling choices can manifest as a spurious redshift trend. We therefore record the low-redshift slope as constraining this falsifier rather than deciding it. The pure-sample measurement is compatible with the lock, and the anchored measurement exceeds it by an amount that an inter-sample offset of the size the same paper exhibits would supply. Every slope and distance quoted in these two paragraphs is computed in data/lit0818_a0z.txt under the cosmology stated there.
...
The third is the redshift axis, and it separates the predictions where the present-day values do not settle the matter. His text states that scaling by a_dS alone would give da₀/dz|_{z=0} > 0, that the curvature dependence through q(z) reverses the sign to da₀/dz|_{z=0} < 0, and that the derivative approaches zero near z ≈ 0.5. The horizon reading tested here predicts a₀ ∝ H(z), which rises. Ciocan et al. (2026) measure a₀ rising with redshift at z ≈ 1, and note that the measured evolution is faster than that of H(z). The low-redshift fit of Vărăşteanu et al. (2026) prefers the same sign in its anchored form, and its pure-sample form is consistent with both no evolution and the lock's own slope (Section 5). Both published determinations of a₀(z) therefore point in the direction the horizon reading predicts and against a falling derivative, so the epoch axis is now populated at two epochs, z ≈ 0.09 and z ≈ 1, rather than none.
...
2. a₀ ∝ H(z): the horizon reading ties the acceleration scale to the epoch. A first level-grade point exists (Section 5): the published z ≈ 1 intercepts of Ciocan et al. (2026) sit 0.7σ and 0.1σ from cH₀/2π under two of their three analysis frameworks and 2.3σ from the third. A first direct slope now exists as well, at low redshift, and it constrains without deciding (Section 5): the pure HI-selected fit of Vărăşteanu et al. (2026) contains both no evolution and the lock's predicted a₁ = +0.52 × 10⁻¹⁰ m s⁻² per unit redshift inside one sigma, while their SPARC-anchored fit sits 4.5σ above the lock with an inter-sample zero-point offset, flagged by the authors, of the size required to produce it. The z ≈ 1 slope leg remains blocked by the release rather than by the data, since the per-point uncertainties needed to reconstruct it are not distributed. Deeper high-redshift rotation curves at JWST and ALMA depth, an intra-sample redshift lever with a single selection function, or an author-supplied release, decide it. This axis now also discriminates against a rival parameter-free derivation of the same constant, which predicts the opposite sign, da₀/dz|_{z=0} < 0 (van Putten 2026; Section 5.1).
...
- the acceleration-scale ladder: stage5l_ladder.py, stage4v_scorecard.py; the z ≈ 1 comparison: stage10h_zladder.py, stage10h_addendum.py (release fetched by fetch_musedark3_release.py); the z ≈ 0.09 slope comparison: data/lit0818_a0z.py (output data/lit0818_a0z.txt)
