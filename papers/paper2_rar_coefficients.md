...
The ledger's current state is quickly summarized. On the galaxy side, the occupation function and its sharpened-tail variants lead the hierarchical treatments; the quarter-coefficient variant is excluded once the vertical channel is active; the simple function trails the hierarchical ladder. No candidate resolves the Section 4 tension (deep ½ with a slightly sharper tail) at full strength in a single fixed form. On the binary side, the companion paper's landed model erases all discrimination: all thirteen laws (and three additional consistency rows) sit within ±8 log-likelihood of the occupation function, with seed scatter of the same size, while Newton loses on every function-realization row by at least 7.9. We regard this as a field-level methods finding: at present, wide binaries do not select among modified-dynamics interpolating functions at all; rotation curves are the only function discriminator, and claims in the literature that binary data prefer one function over another should be examined for fence conditions of the kind our own earlier indication carried. Outside the family, two brackets close the ledger: no standard-family function survives both the measured wide-binary window and its own published solar-system verdict, and the ΛCDM radial-acceleration attractor of current simulations does not print a zero-point digit at all, so the c₁ dial is, at present, a modified-dynamics observable.

## 8. The gas-dominated anomaly

A referee-grade robustness row returned a discovery instead. Measuring the coefficient dial on gas-dominated galaxies, where the stellar mass-to-light nuisance nearly vanishes, was intended as the clean confirmation; it returned a sign reversal. Under the treatment with measured distance and inclination priors, the 38 gas-dominated galaxies fit ĉ₁ ≈ −0.66 where the 111 disk-dominated galaxies fit +0.67. The full-sample headline of Section 3 is a disk-dominated compromise, and a negative c₁ is outside every function in the ledger, ours included.
